...
- ``mail:imap_watermark`` — last seen IMAP UID (migrated from imap:last_uid)

Legacy credential migration
----------------------------
Use :func:`migrate_legacy_credentials` to migrate existing ``caldav`` /
``imap`` tool_config rows to the unified ``mail`` namespace.  The function is
...
# ---------------------------------------------------------------------------
# Legacy key names from the old separate capabilities
# ---------------------------------------------------------------------------

_LEGACY_IMAP_EMAIL = "imap:email"
_LEGACY_IMAP_PASSWORD = "imap:password"
_LEGACY_IMAP_WATERMARK = "imap:last_uid"
_LEGACY_CALDAV_USERNAME = "caldav:username"
_LEGACY_CALDAV_PASSWORD = "caldav:password"


# ---------------------------------------------------------------------------
# Module-level migration function
# ---------------------------------------------------------------------------


def migrate_legacy_credentials() -> bool:
    """Migrate legacy caldav/imap credentials to unified mail:* keys.

    Reads existing ``tool_configs`` rows under ``tool_name='imap'`` or
    ``tool_name='caldav'``, re-encrypts the credentials under
    ``tool_name='mail'``, updates ``scheduled_items.source`` references,
    and removes the old rows.

    Idempotent — skips if ``mail:email`` already exists.

    The vault must be unlocked before calling this function.  Any
    cryptographic errors propagate to the caller.

    Returns:
        True if migration was performed, False if skipped (already migrated
        or no legacy credentials found).
    """
    ...
    tc = ToolConfigService(db)
    vault = get_vault_service()

    # -----------------------------------------------------------------------
    # Idempotency guard: if mail:email already exists, skip migration entirely
    # -----------------------------------------------------------------------
    mail_cfg = tc.get_tool_config("mail")
    if mail_cfg.get(_K_EMAIL):
        logger.debug("[mail migration] mail:email already present — skipping")
        return False

    # -----------------------------------------------------------------------
    # Step 1: Collect raw (encrypted) values from legacy tool_config rows
    # -----------------------------------------------------------------------
    imap_cfg = tc.get_tool_config("imap")
    caldav_cfg = tc.get_tool_config("caldav")

    # Prefer IMAP credentials for the email address; fall back to CalDAV username
    email_encrypted: str | None = imap_cfg.get(_LEGACY_IMAP_EMAIL)
    password_encrypted: str | None = imap_cfg.get(_LEGACY_IMAP_PASSWORD)
    watermark_raw: str | None = imap_cfg.get(_LEGACY_IMAP_WATERMARK)

    source_was = "imap"

    if email_encrypted is None:
        # Try CalDAV credentials (username = email for Google/Apple/Yahoo)
        email_encrypted = caldav_cfg.get(_LEGACY_CALDAV_USERNAME)
        password_encrypted = caldav_cfg.get(_LEGACY_CALDAV_PASSWORD)
        source_was = "caldav"

    if email_encrypted is None:
        logger.debug("[mail migration] no legacy credentials found — skipping")
        return False

    # -----------------------------------------------------------------------
    # Step 2: Decrypt legacy values so we can resolve the provider
    # -----------------------------------------------------------------------
    def _decrypt(blob: str) -> str | None:
        """Base64-decode then AES-GCM decrypt an encrypted config_value."""
        try:
            raw = base64.b64decode(blob.encode())
            return vault.decrypt_str(raw)
        except Exception as exc:  # noqa: BLE001
            logger.warning("[mail migration] decryption failed: %s", exc)
            return None

    email_plain = _decrypt(email_encrypted)
    if not email_plain:
        logger.warning("[mail migration] could not decrypt email address — aborting")
        return False

    password_plain: str | None = None
    if password_encrypted:
        password_plain = _decrypt(password_encrypted)

    # -----------------------------------------------------------------------
    # Step 3: Resolve provider from email address
    # -----------------------------------------------------------------------
    provider = discover_provider(email_plain)

    # -----------------------------------------------------------------------
    # Step 4: Re-encrypt and write unified mail:* credentials
    # -----------------------------------------------------------------------
    def _encrypt(plaintext: str) -> str:
        """AES-GCM encrypt then base64-encode for storage."""
        blob = vault.encrypt_str(plaintext)
        return base64.b64encode(blob).decode()

    new_config: dict[str, str] = {
        _K_EMAIL: _encrypt(email_plain),
    }

    if password_plain:
        new_config[_K_PASSWORD] = _encrypt(password_plain)

    if provider:
        new_config[_K_PROVIDER] = _encrypt(provider.name)

        if provider.imap:
            new_config[_K_IMAP_HOST] = _encrypt(provider.imap.host)
            new_config[_K_IMAP_PORT] = _encrypt(str(provider.imap.port))
            new_config[_K_IMAP_TLS] = _encrypt(str(int(provider.imap.tls)))

        if provider.smtp:
            new_config[_K_SMTP_HOST] = _encrypt(provider.smtp.host)
            new_config[_K_SMTP_PORT] = _encrypt(str(provider.smtp.port))
            new_config[_K_SMTP_TLS] = _encrypt(str(int(provider.smtp.tls)))

        if provider.caldav_url:
            new_config[_K_CALDAV_URL] = _encrypt(provider.caldav_url)

        if provider.carddav_url:
            new_config[_K_CARDDAV_URL] = _encrypt(provider.carddav_url)

        # Build protocols list based on what the provider supports
        protocols: list[str] = []
        if provider.imap:
        ...
        new_config[_K_PROTOCOLS] = _encrypt(json.dumps(protocols))

    # Migrate IMAP watermark if present
    if watermark_raw:
        # watermark_raw is a plain integer string, not encrypted — copy as-is
        # (old imap capability stores it via store_credential which encrypts it)
        watermark_plain = _decrypt(watermark_raw)
        if watermark_plain:
            new_config[_K_IMAP_WATERMARK] = _encrypt(watermark_plain)

    tc.set_tool_config("mail", new_config)

    # -----------------------------------------------------------------------
    # Step 5: Update scheduled_items.source references
    # -----------------------------------------------------------------------
    _update_scheduled_items_source(db)

    # -----------------------------------------------------------------------
    # Step 6: Delete old tool_config rows
    # -----------------------------------------------------------------------
    _sources_to_delete = set()
    if imap_cfg:
        _sources_to_delete.add("imap")
    if caldav_cfg:
        _sources_to_delete.add("caldav")

    for tool_name in _sources_to_delete:
        tc.delete_tool_config(tool_name)

    logger.info(
        "[mail migration] migrated legacy %s credentials for %s → mail",
        source_was,
        email_plain,
    )
    return True
...
def _update_scheduled_items_source(db) -> None:
    """Update scheduled_items.source from 'caldav'/'imap' to 'mail'."""
    try:
        with db.connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE scheduled_items SET source = 'mail' WHERE source IN ('caldav', 'imap')"
            )
            updated = cursor.rowcount
            cursor.close()
        if updated:
            logger.info("[mail migration] updated %d scheduled_items rows source → mail", updated)
    except Exception as exc:  # noqa: BLE001
        logger.warning("[mail migration] scheduled_items source update failed: %s", exc)
