...
class HaRestHandler:
    ...
    def _get(url: str, path: str, token: str, verify_ssl: bool) -> requests.Response:
        resp = requests.get(
            f"{url.rstrip('/')}{path}",
            headers=HaRestHandler._headers(token),
            verify=verify_ssl,
            timeout=_TIMEOUT,
        )
    ...
    def probe(url: str, token: str, verify_ssl: bool) -> dict[str, object]:
        return cast("dict[str, object]", HaRestHandler._get(url, "/api/", token, verify_ssl).json())
    ...
    def list_devices(
        ...
        states = HaRestHandler._get(url, "/api/states", token, verify_ssl).json()
        if domain:
            states = [s for s in states if s["entity_id"].startswith(f"{domain}.")]

        if area:
            try:
                areas = HaRestHandler._get(url, "/api/config/area_registry/list", token, verify_ssl).json()
    ...
    def get_state(url: str, token: str, verify_ssl: bool, entity_id: str) -> dict[str, object]:
        data = HaRestHandler._get(url, f"/api/states/{entity_id}", token, verify_ssl).json()
    ...
    def control(
        ...
        HaRestHandler._post(url, f"/api/services/{domain}/{service}", token, verify_ssl, body)
