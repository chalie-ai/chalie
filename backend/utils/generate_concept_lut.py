...
class ConceptLutBuilder:
    ...
    def _read_concepts(self, yaml_path: Path) -> list[dict[str, str]]:
        with open(yaml_path) as f:
            data = yaml.safe_load(f)
        raw_concepts = cast(list[dict[str, str]], data.get("concepts", []))
        if not raw_concepts:
    ...
    def _insert_concepts(self, conn: sqlite3.Connection, concepts: list[dict[str, str]]) -> list[int]:
        ids = []
        for c in concepts:
            conn.execute(
...
def main() -> None:
    ...
    builder = ConceptLutBuilder()
    concepts = builder._read_concepts(_YAML_PATH)
    ...
    try:
        builder._load_sqlite_vec(conn)
        builder._rebuild_tables(conn)
        ids = builder._insert_concepts(conn, concepts)
        inserted = builder._embed_and_insert(conn, emb_service, concepts, ids)
    finally:
        conn.close()
