...
class SkillsIO:
    ...
    @classmethod
    def skill_yaml_path(cls, title: str) -> Path:
        """Return the YAML file path for a user skill."""
        return FileMapperService.get_user_skills_path(f"{cls.slugify_title(title)}.yaml")

    @classmethod
    def ensure_user_skills_dir(cls) -> None:
    ...
    @classmethod
    def write_skill_file(cls, path: Path, meta: dict[str, str | int]) -> None:
        ...
        body = cast("str", meta.get("content", ""))
        content = (
            "---\n"
    ...
    @classmethod
    def load_sqlite_vec(cls, conn: sqlite3.Connection) -> None:
    ...
    @classmethod
    def open_skills_db(cls, *, row_factory: bool = False) -> sqlite3.Connection:
        ...
        cls.load_sqlite_vec(conn)
        return conn

    @classmethod
    def remove_search_entries(cls, conn: sqlite3.Connection, skill_id: int) -> None:
