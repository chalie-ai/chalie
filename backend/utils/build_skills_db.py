...
Walks backend/abilities/skills/ for .yaml skill files, embeds title + use_for
text for each, and writes:
  backend/abilities/assets/skills.sqlite  — vector + FTS5 search index
  resources/pre-trained/skills_sha.json   — drift sidecar
...
def _parse_skill_file(path: Path) -> dict:
    """Parse a YAML-frontmatter + Markdown skill file into a metadata dict."""
    text = path.read_text()
...
def _insert_skill(
    ...
    """Insert one skill and its search entries. Returns count of entries inserted."""
    ...
    conn.execute(
        "INSERT INTO skills(title, use_for, content, tags, version, related_abilities) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        (
            title,
            use_for,
    ...
    # Embed combined title + use_for text (not tags individually).
    combined = f"{title}. {use_for}"
    raw_entries: list[tuple[str, str]] = [
        (title, 'title'),
        (use_for, 'use_for'),
        (combined, 'use_for'),  # combined centroid
    ]
...
def _build(db_path: Path, sha_path: Path) -> None:
    skills = _load_skills()
    print(f"Found {len(skills)} skills — building {db_path.name}...")

    emb_service = EmbeddingService() if skills else None
    ...
    try:
        ...
        total_entries = 0
        for meta in skills:
            n = _insert_skill(conn, emb_service, meta)
            print(f"  {meta.get('title', meta['_path'])}: {n} entries")
            total_entries += n
    finally:
        conn.close()

    sha_map = _build_sha_map(skills)
    sha_path.write_text(json.dumps(sha_map, indent=2, sort_keys=True) + "\n")

    print(f"\nDone. {len(skills)} skills, {total_entries} entries total.")
