...
class SkillsDbBuilder:
    ...
    def _parse_skill_file(self, path: Path) -> dict[str, object]:
        text = path.read_text()
    ...
    def _compute_sha(self, meta: dict[str, object]) -> str:
        raw = json.dumps(
    ...
    def index_skill(
        ...
        entries, embeddings = self._dedup_entries(raw_entries, embeddings)
    ...
    def _insert_skill(
        ...
        return self.index_skill(conn, emb_service, skill_id, title, use_for, tags_str)
    ...
    def _build_sha_map(self, skills: list[dict[str, object]]) -> dict[str, str]:
        return {cast("str", m.get('title', m['_path'])): self._compute_sha(m) for m in skills}

    def _build(self, db_path: Path, sha_path: Path) -> None:
        ...
        """
        curated_skills = self._load_skills()
        ...
        try:
            ...
            for meta in curated_skills:
                n = self._insert_skill(conn, cast("EmbeddingService", emb_service), meta, source=_SOURCE_CURATED)
                print(f"  [curated] {meta.get('title', meta['_path'])}: {n} entries")
                total_entries += n
        finally:
            conn.close()

        sha_map = self._build_sha_map(curated_skills)
...
def main() -> None:
    ...
    builder = SkillsDbBuilder()
    if args.check:
        builder._check(_SHA_PATH)
    else:
        builder._build(_DB_PATH, _SHA_PATH)
