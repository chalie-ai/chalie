...
"""
Reasoning Loop Service — signal-driven continuous reasoning.
...
LOG_PREFIX = "[REASONING LOOP]"

# MemoryStore keys
COOLDOWN_KEY = "reasoning_loop:cooldowns"
FATIGUE_KEY = "reasoning_loop:activations"
STATE_KEY = "reasoning_loop:state"
...
class ReasoningSignal:
    """A signal that triggers reasoning in the reasoning loop."""
...
class ReasoningLoopService:
    ...
    service blocks on blpop() and processes each signal through the full
    spreading-activation → synthesis → action pipeline.

    When no signal arrives within idle_timeout seconds, the service enters
    ...
    SIGNAL_DISPATCH = {
        ...
        'thread_expired': '_handle_thread_expired',
    }
    ...
    def __init__(self, check_interval: int = 300):
        ...
        Args:
            check_interval: Base sleep interval in seconds between reasoning cycle
    ...
    def _process_signal(self, signal: ReasoningSignal) -> None:
        """Dispatch signal to type-specific handler."""
        handler_name = self.SIGNAL_DISPATCH.get(signal.signal_type)
        if handler_name:
    ...
    def _handle_trait_changed(self, signal: ReasoningSignal) -> None:
        ...
        try:
            # Store trait change note for autobiography service to pick up
            note = signal.content or 'trait changed'
            self.store.rpush('reasoning_loop:identity_shifts', json.dumps({
                'content': note[:200],
                'timestamp': signal.timestamp,
                'energy': signal.activation_energy,
            }))
            ...
            logger.debug(f"{LOG_PREFIX} Failed to note trait change: {e}")

    def _handle_task_state_changed(self, signal: ReasoningSignal) -> None:
        """Track goal progress — light state update, escalate completions."""
        try:
            content = signal.content or ''
            self.store.set('reasoning_loop:last_task_event', json.dumps({
                'content': content[:200],
                'timestamp': signal.timestamp,
                'energy': signal.activation_energy,
            }), ex=3600)
            ...
            logger.debug(f"{LOG_PREFIX} Failed to track task state: {e}")
    ...
    def _handle_schedule_fired(self, signal: ReasoningSignal) -> None:
        ...
        try:
            ...
            logger.debug(f"{LOG_PREFIX} Failed to handle schedule fire: {e}")

    def _handle_thread_expired(self, signal: ReasoningSignal) -> None:
        """Note thread expiry — extract potential unresolved goals."""
        try:
            content = signal.content or ''
            self.store.rpush('reasoning_loop:expired_threads', json.dumps({
                'content': content[:200],
                'timestamp': signal.timestamp,
            }))
            # Cap at 10 entries
            self.store.ltrim('reasoning_loop:expired_threads', -10, -1)
    ...
    def _handle_idle_signal(self) -> None:
        ...
        # Gate checks (same as _handle_reasoning_signal)
        if self._is_fatigued():
            return
        if self._is_user_deep_focus():
