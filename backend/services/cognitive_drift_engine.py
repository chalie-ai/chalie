...
class CognitiveDriftEngine:
    ...
    def run(self, shared_state: Optional[dict] = None) -> None:
        ...
        Each iteration: processes any deferred thoughts, sleeps for a jittered
        interval, checks worker-idle and episode-availability preconditions,
        verifies memory richness, then executes one drift cycle. Restarts
        automatically after non-fatal errors with a 60-second back-off.

        Args:
            shared_state: Optional shared state dict passed by the worker
                harness. Not used directly by the engine, but accepted for
