...
class TurnSerializerService:
    ...
    def _fetch_tool_calls_for_transcripts(self, transcript_ids: list[int]) -> list[dict[str, object]]:
    ...
    def _fetch_attachments_for_transcripts(self, conn: sqlite3.Connection, transcript_ids: list[int]) -> dict[int, list[dict[str, object]]]:
    ...
    def _group_calls_by_transcript(self, calls: list[dict[str, object]]) -> dict[int, list[dict[str, object]]]:
    ...
    def _base_message(self, r: dict[str, object]) -> dict[str, object]:
    ...
    def _apply_user_fields(self, msg: dict[str, object], r: dict[str, object], attachments_by_id: dict[int, list[dict[str, object]]]) -> None:
        attachments = attachments_by_id.get(cast("int", r['id']))
        if attachments:
            msg["attachments"] = attachments

    def _tool_call_chips(self, own_calls: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    def _voice_states(self, rows: list[dict[str, object]]) -> dict[int, str]:
    ...
    def _thinking_states(self, rows: list[dict[str, object]]) -> dict[int, list[dict[str, object]]]:
    ...
    def _rows_to_messages(self, rows: list[dict[str, object]]) -> list[dict[str, object]]:
        ...
        conn = Database.conn()
        attachments_by_id = self._fetch_attachments_for_transcripts(
        ...
        turn_calls = self._fetch_tool_calls_for_transcripts(turn_scope_ids)
        # Group by transcript_id for per-row chip lookup.
        calls_by_transcript = self._group_calls_by_transcript(turn_calls)
        ...
        voice_states = self._voice_states(rows)
        thinking_states = self._thinking_states(rows)
        cycle_calls: list[dict[str, object]] = []
        for r in rows:
            msg = self._base_message(r)
            own = calls_by_transcript.get(cast("int", r['id']), [])
            chips = self._tool_call_chips(own)
            if chips:
            ...
            if r['role'] == 'user':
                ...
                self._apply_user_fields(msg, r, attachments_by_id)
            else:
                cycle_calls = cycle_calls + own
                self._apply_assistant_fields(msg, cycle_calls)
    ...
    def _bulk_gists(self, channel: str, turn_ids: list[int]) -> dict[int, str]:
    ...
    def _drop_trailing_cancelled_orphan(
        self, rows: list[dict[str, object]], latest: TurnExecution | None,
    ...
    def serialize(self, channel: str, turn_id: int, config_type: str = _TYPE) -> dict[str, object]:
        ...
        rows = self._drop_trailing_cancelled_orphan(rows, latest)
        ...
        return {
            "turn_id": turn_id,
            "gist": self._bulk_gists(channel, [turn_id]).get(turn_id),
    ...
    def bulk_gists(self, channel: str, turn_ids: list[int]) -> dict[int, str]:
        return self._bulk_gists(channel, turn_ids)
