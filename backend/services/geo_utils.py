...
_geodesic: object = None


class GeoUtils:
    """Stateless geo utility functions."""

    DEFAULT_SPEED_KMH: ClassVar[float] = 30  # conservative fallback (urban mixed traffic)
    _MIN_SPEED_KMH: ClassVar[float] = 1.0   # guard against GPS noise yielding absurd speeds
    _MAX_SPEED_KMH: ClassVar[float] = 200.0  # guard against GPS jumps
    _PARSE_UTC_SENTINEL: ClassVar[datetime] = datetime.min.replace(tzinfo=timezone.utc)
    ...
    def estimate_speed_from_history(history_entries: list[object]) -> float | None:
        ...
        for entry in history_entries:
            ...
            try:
                ...
                continue
            ts = parse_utc(ts_raw)
            if ts == GeoUtils._PARSE_UTC_SENTINEL:
                continue
        ...
        for i in range(1, len(valid)):
            ...
            if elapsed_hours <= 0:
                continue
            dist = GeoUtils.distance_km(prev_lat, prev_lon, curr_lat, curr_lon)
            spd = dist / elapsed_hours
            if GeoUtils._MIN_SPEED_KMH <= spd <= GeoUtils._MAX_SPEED_KMH:
                speeds.append(spd)
