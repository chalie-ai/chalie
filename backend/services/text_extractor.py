...
class TextExtractor:
    """Pure text-extraction utilities.  All methods are ``@staticmethod`` so
    the class carries no state — it is a namespace, not a service."""
    ...
    def extract_text(file_path: str, mime_type: str | None = None) -> str:
        ...
        """
        if not mime_type:
            mime_type = TextExtractor.detect_mime_type(file_path)
        ...
        extractors: dict[str, Callable[[str], str]] = {
            'application/pdf': TextExtractor._extract_pdf,
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document': TextExtractor._extract_docx,
            'application/vnd.openxmlformats-officedocument.presentationml.presentation': TextExtractor._extract_pptx,
            'text/html': TextExtractor._extract_html_file,
            'text/plain': TextExtractor._extract_plain,
            'text/markdown': TextExtractor._extract_plain,
        }
        ...
        if mime_type.startswith('text/'):
            return TextExtractor._extract_plain(file_path)
    ...
    def _extract_pdf(path: str) -> str:
        ...
        try:
            ...
            logger.exception('[TEXT EXTRACTOR] pdfplumber not installed — cannot extract PDF')
            return ''
    ...
    def _extract_docx(path: str) -> str:
        ...
        try:
            ...
            logger.exception('[TEXT EXTRACTOR] python-docx not installed — cannot extract DOCX')
            return ''
    ...
    def _extract_pptx(path: str) -> str:
        ...
        try:
            ...
            logger.exception('[TEXT EXTRACTOR] python-pptx not installed — cannot extract PPTX')
            return ''
    ...
    def _extract_html_file(path: str) -> str:
        ...
        try:
            with open(path, 'r', encoding='utf-8', errors='replace') as f:
                raw = f.read()
            return TextExtractor.extract_html(raw)
        except Exception as e:
