...
class ThinkingLevelClassifierService:
    ...
    Fallback: sticky prev_level (or 'low' on cold start).
    All exceptions are silently trapped — gate failure must never kill a turn.

    The classifier is the sole decision maker. No anti-demotion guards, no
    channel counters. Whatever the head returns above the confidence threshold
    is the answer. Below threshold: sticky fallback to prev_level, or 'low'
    on cold start.
    """
    ...
    def classify(self, user_turn: str, prev_level: str = 'none') -> dict:
        ...
        try:
            ...
            if label is None or confidence < _CONFIDENCE_THRESHOLD:
                ...
                # answer without deliberation pressure. 'medium' was the previous
                # default but it is an ACTIVE intervention, not a neutral default;
                # applying it whenever classifier confidence dropped below threshold
                # caused spurious deliberation on simple recall/chit-chat turns.
            ...
            # 'low' (no-op) on cold start, not 'medium' (active deliberation).
            fallback_level = prev_level if prev_level != 'none' else 'low'
            logger.info(
