...
class CommunicateAction(AutonomousAction):
    ...
    def _add_candidate(self, thought: ThoughtContext, score: float):
        ...
        self.store.zadd(candidates_key, {json.dumps(candidate): score})

        # Trim to max candidates (keep highest scoring)
        count = self.store.zcard(candidates_key)
        if count > self.max_candidates:
            self.store.zremrangebyrank(candidates_key, 0, count - self.max_candidates - 1)

        # TTL on the set = max gist TTL
        self.store.expire(candidates_key, thought.drift_gist_ttl)

        logger.info(
            f"{LOG_PREFIX} Added candidate: [{thought.thought_type}] "
            f"score={score:.3f} ({self.store.zcard(candidates_key)} in queue)"
        )
    ...
    def _get_best_candidate(self) -> Optional[Dict]:
        ...
        for raw, _stored_score in raw_members:
            ...
            # Expired candidate
            if elapsed >= ttl:
                self.store.zrem(candidates_key, raw)
                continue
    ...
    def _add_deferred(self, thought: ThoughtContext, score: float):
        ...
        self.store.zadd(deferred_key, {json.dumps(deferred): score})

        # Trim to max
        count = self.store.zcard(deferred_key)
        if count > self.max_deferred:
            self.store.zremrangebyrank(deferred_key, 0, count - self.max_deferred - 1)

        self.store.expire(deferred_key, self.deferred_ttl)
    ...
    def process_deferred_queue(self) -> Optional[Dict]:
        ...
        raw_members = self.store.zrange(deferred_key, 0, -1, withscores=True)
        if not raw_members:
            return None

        # Top thought gets delivery attempt
        best_raw, _score = raw_members[-1]  # Highest score
        self.store.zrem(deferred_key, best_raw)
        ...
        self.store.delete(deferred_key)

        # Mark as processed for this cycle (expire after quiet hours window)
        self.store.setex(processed_key, 43200, '1')  # 12h TTL
    ...
    def record_user_interaction(self, message_embedding: list = None):
        ...
        if message_embedding:
            ...
            self.store.rpush(embeddings_key, entry)
            self.store.ltrim(embeddings_key, -20, -1)  # Keep last 20
            self.store.expire(embeddings_key, self.max_idle_seconds)  # TTL = max idle
