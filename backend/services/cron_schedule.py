...
class CronSchedule:
    """Stateless crontab engine with five numeric fields (minute, hour, dom,
    month, dow).

    All entry points are static methods so callers import a single name and
    the module can grow without exposing internals.
    """
    ...
    def parse_field(expr: str, lo: int, hi: int) -> frozenset[int]:
        ...
        for token in expr.split(","):
            ...
            values |= CronSchedule._parse_token(token, lo, hi)
    ...
    def validate_cron(
        ...
        """
        CronSchedule._validate_one("minute", minute, 0, 59)
        CronSchedule._validate_one("hour", hour, 0, 23)
        CronSchedule._validate_one("dom", dom, 1, 31)
        CronSchedule._validate_one("month", month, 1, 12)
        CronSchedule._validate_one("dow", dow, 0, 7)

    @staticmethod
    def _validate_one(name: str, expr: str, lo: int, hi: int) -> None:
        try:
            CronSchedule.parse_field(expr, lo, hi)
    ...
    def matches(
        ...
        minset = CronSchedule.parse_field(minute, 0, 59)
        hourset = CronSchedule.parse_field(hour, 0, 23)
        domset = CronSchedule.parse_field(dom, 1, 31)
        monthset = CronSchedule.parse_field(month, 1, 12)
        dowset = CronSchedule.parse_field(dow, 0, 7)
