...
class SchemaConvergenceService:
    ...
    def __init__(self, db_service, embedding_dimensions: int = 768):
        ...
        self._seeder_path = Path(__file__).resolve().parent.parent / "seeder.sql"
    ...
    def _run_seeder(self, live_conn: sqlite3.Connection) -> None:
        """Execute seeder.sql statements on existing databases.

        This replaces the migration-file-scanning logic with a single
        idempotent seeder file.
        """
        if not self._seeder_path.exists():
            return

        sql = self._seeder_path.read_text()
        sql_no_comments = re.sub(r"--[^\n]*", "", sql)
        statements = [s.strip() for s in sql_no_comments.split(";") if s.strip()]
        ...
        for stmt in statements:
            try:
                ...
                if any(pat in err_msg for pat in _IDEMPOTENT_ERRORS):
                    logger.debug(f"[convergence] Skipping already-applied step in seeder: {exc}")
                else:
                    logger.warning(f"[convergence] Seeder statement failed: {exc}")
