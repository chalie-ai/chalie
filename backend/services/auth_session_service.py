...
class AuthSessionService:
    """Cookie-based session management backed by MemoryStore and SQLite."""
    ...
    @classmethod
    def _persist_session_to_sqlite(cls, token: str) -> None:
        ...
        try:
            ...
            with Database.transaction() as conn:
                conn.execute(
                    ...
                    (cls._hash_session_token(token), utc_now().isoformat())
                )
        except Exception as e:
            logger.exception(f"[Session] SQLite persist failed: {e}")

    @classmethod
    def _delete_session_from_sqlite(cls, token: str) -> None:
        """Remove session from SQLite."""
        try:
            from services.database import Database
            with Database.transaction() as conn:
                conn.execute("DELETE FROM auth_sessions WHERE token = ?", (cls._hash_session_token(token),))
        except Exception as e:
            logger.debug(f"[Session] SQLite delete failed: {e}")

    @classmethod
    def _validate_session_in_sqlite(cls, token: str) -> bool:
    ...
    @classmethod
    def create_session(cls, response: Response) -> str:
        ...
        cls._persist_session_to_sqlite(token)
        ...
        response.set_cookie(
            ...
            httponly=True,
            samesite='Lax',
            secure=cls._cookie_secure(),
        )
        logger.info("[Session] Created new session")
        return token

    @classmethod
    def validate_session(cls, request: Request) -> bool:
        ...
        return cls._validate_session_in_sqlite(token)

    @classmethod
    def destroy_session(cls, request: Request, response: Response) -> None:
        ...
        if token:
            ...
            cls._delete_session_from_sqlite(token)
