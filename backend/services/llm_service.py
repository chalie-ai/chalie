...
# ── Message converters (kept for backward-compat; consumed by thin clients) ──

def _anthropic_convert_messages(messages: list) -> list:
    """Convert normalized messages to Anthropic's content block format.

    Kept here for backward-compat with tests that import from llm_service.
    The canonical implementation lives in llm_clients/anthropic.py.
    """
    from services.llm_clients.anthropic import _anthropic_convert_messages as _impl  # noqa: PLC0415
...
def _openai_convert_messages(messages: list) -> list:
    ...
    Kept here for backward-compat with tests that import from llm_service.
    """
    from services.llm_clients.openai import _openai_convert_messages as _impl  # noqa: PLC0415
    return _impl(messages)
...
def _gemini_convert_messages(messages: list) -> list:
    ...
    Kept here for backward-compat with tests that import from llm_service.
    """
    from services.llm_clients.gemini import _gemini_convert_messages as _impl  # noqa: PLC0415
    return _impl(messages)
...
def _gemini_accumulate_part(part, text_parts: list, tool_calls: list) -> None:
    ...
    Kept here for backward-compat with tests that import from llm_service.
    """
    from services.llm_clients.gemini import _accumulate_part  # noqa: PLC0415
    _accumulate_part(part, text_parts, tool_calls)


# ── Backward-compat shims ──────────────────────────────────────────────────────
#
# Tests import these from services.llm_service; tester agent should migrate
# those imports to services.provider_api.  Shims remain until that migration.

def __getattr__(name):
    """Lazy shims for symbols moved to services.provider_api."""
    _moved = {
        'LLMResponse': 'ProviderApiResponse',
        'RateLimitError': 'RateLimitError',
        'PayloadTooLargeError': 'ResponseOverLimitError',
        'NonRetryableError': 'ProviderResponseError',
    }
    if name in _moved:
        import importlib
        mod = importlib.import_module('services.provider_api')
        return getattr(mod, _moved[name])
    raise AttributeError(f"module 'services.llm_service' has no attribute {name!r}")
