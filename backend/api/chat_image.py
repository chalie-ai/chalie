...
def _run_analysis(image_id: str, image_bytes: bytes, mime_type: str):
    ...
    - Broadcasts ``{"type": "image_ready", "image_id": ..., "status": ...}`` via
      WebSocketBroker to connected clients (Step 3 / B5 fix).
    ...
    try:
        ...
        # Push analysis completion to connected WebSocket clients.
        try:
            from services.websocket_broker import WebSocketBroker
            WebSocketBroker().broadcast({'type': 'image_ready', 'image_id': image_id, 'status': 'ready'})
        except Exception:
            pass
        logger.info(
        ...
        # Push failure event so the frontend can show an error badge.
        try:
            from services.websocket_broker import WebSocketBroker
            WebSocketBroker().broadcast({'type': 'image_ready', 'image_id': image_id, 'status': 'failed'})
        except Exception:
