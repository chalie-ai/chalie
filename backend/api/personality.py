...
@require_session
def get_personality() -> "ResponseReturnValue":
    try:
        from services.personality.personality_service import personality_service

        tup = personality_service.get_tuple()
        return jsonify({"tuple": list(cast("tuple[int, int, int, int, int]", tup)), "voice": personality_service.get_voice()}), 200
...
@require_session
def set_personality() -> "ResponseReturnValue":
    try:
