...
> - **Test cleanup:** landed — old-spine test coupling dropping (message_processor test imports 75→1, `Transcript` classmethods 12→0, execution_tracker 1→0, act_trail 31→21).
...
> - **DB gateway (Finding-1):** ~90% consolidated; 2 non-carve-out `get_shared_db_service` sites left. See §3.7 STATUS.
...
- **One-class-per-file (Essential 3):** "Exempt enums+exceptions only" — a cohesive enum group and an exception taxonomy may each stay in one file; **everything else splits one class per file** (including the prompt-fragment subclasses).
...
> - **Residual (not a violation):** raw `Websocket.broadcast` + self-gate in `services/act_trail.py` — dead code in a phase-2 file (0 prod importers), cleared when it is deleted post test-migration. (`services/execution_tracker.py` — the other former residual — is now DELETED, jul05.)
...
- **One class per file — with ONE exemption (Dylan ruling).** Split every class into its own file, **except**: a cohesive *enum group* and an *exception taxonomy* may each stay grouped in one file (`models/provider_enums.py`, `models/provider_errors.py`). Everything else — including each prompt-fragment subclass — is one class per file.
...
| `SystemMessagePrompt` (base) + 8 subclasses (incl. `getPrompt` back-compat shim) | services/system_message_prompt.py | **Kept, de-shimmed, SPLIT** | prompt-text producers; delete the `getPrompt` alias (shim); **split to one class per file** under their module (Essential 3 / Q_B — the exemption covers only enums+exceptions, not these); called by `PromptService`, never by configs |
...
| `ConfigTypeEnum` | (config module) | **Kept** as a routing enum (grouped-enum exemption, §2.3) | maps `type` → config; read via `self.mp.config`, never `.get_by_type` from a service body |
...
| Class | File:line | Fate | Reason |
|---|---|---|---|
| `ToolCallState` | services/act_trail.py:46 | **Folded into** `models/tool_call.py` (state consts on the model) | Essential 3 |
| `ToolCall` (frozen dataclass) | services/act_trail.py:54 | **Rewritten** → `models/tool_call.py::ToolCall` (active-record) | Rule 5; keep WS-safe `to_json` (params/result NEVER on wire — §6.2) |
| `ActTrail` | services/act_trail.py:100 | **Dissolved** → `services/tool_call_service.py::ToolCallService` | raw SQL + `WebSocketBroker()` (L259) + `get_shared_db_service()` default; bare-ctor read-only mode removed |
...
| `ProviderType`, `ThinkingLevel` (enums) | provider_api.py:8/15 | **Kept, grouped** → `models/provider_enums.py` | routing/thinking enums — enum-group exemption (§2.3) |
...
> **STATUS (evidence sweep, jul05) — the "~45 sites" estimate has drifted DOWN; consolidation is ~90% done.** `services/database.py::Database.conn()` (L61) + `Database.transaction()` (L117, reentrant `BEGIN IMMEDIATE`) already exist as `@staticmethod` and carry the **overwhelming majority** of DB access (~200+ call sites across `run.py`, `api/`, `abilities/`, `configs/channels/`, and most of `services/`; on-spine via `self.mp.db.transaction()` — `message_processor.py:183`, `tool_call_service.py:63/132`, `transcript_service.py:242`, `turn_execution_service.py:167`). What actually remains on the OLD `get_shared_db_service`/`DatabaseService` path:
> - **16 production `get_shared_db_service()` call sites — 13 are the authorized auth/vault carve-out** (LEAVE): `user_auth.py:102/161/200/288`, `auth_session_service.py:36/50/60/151`, `wrapper_auth_service.py:25` (+ feeders `auth.py:66`, `websocket.py:61`, `wrappers.py:40`), `vault_service.py:565`. **⚠ drift:** the carve-out was speced as "one authorized `user_auth.py` line" but is now **4** lines (`text` shim + `get_session()`) — decide whether to bless 4 or reduce.
> - **Only 2 non-carve-out sites left to migrate** to `Database.conn()`/`transaction()`: `api/mcp_settings.py:60`, `mcp_server/server.py:178`. (The third — `services/execution_tracker.py:149` — is gone: the whole file is DELETED, jul05, §3.4.) Note: `mcp_server/server.py` already uses `Database.transaction()` in its live token path (L80/93); the `:178` residue is a `SettingsService` read that follows the settings-service migration.
> - **1** `DatabaseService(` instantiation, internal to the to-be-deleted singleton (`database_service.py:55`).
> - **0** `self.mp.database_service`/`self.database_service` reads; **0** `self.db`→`DatabaseService` holders (`message_processor.py:144` `self.db = Database()` is the static gateway, correct).
> - **5 stale `dgs.db.connection()` references** to a now-undefined attribute (`DataGraphService` no longer defines `.db`): `abilities/document.py:341`, `api/documents.py:346/395`, `services/document_service.py:453/490` — **latent breakage**, tie into the DataGraphService/model-completion pass.
>
> **Net remaining work for this section: 3 call-site migrations + 5 stale-ref fixes + delete the singleton/shims (G4).** Not ~45.
...
| `models/provider_enums.py` | `ProviderType`, `ThinkingLevel` | no | grouped enums (enum-group exemption, §2.3). |
...
- Provider enums + error taxonomy (grouped, §3.6); thin `llm_clients/*` (transport).
...
services/provider_api.py::{ProviderType,ThinkingLevel} → models/provider_enums.py
...
- **B2 · Transient models.** OWN: `models/ws_message.py`, `models/turn_signal.py`, `models/error_frame.py`, `models/provider_request.py`, `models/provider_response.py`, `models/provider_enums.py`, `models/provider_errors.py`. NO `ToolCallFrame`/`TurnExecutionFrame` (§3.5). STOP-LINE: no persistence (`no save`), no mp.
...
> **RESOLVED (Dylan, this session):** `MessageProcessor` lives in `controllers/message_processor.py`; lowercase dirs + snake_case files + PascalCase classes spine-wide (§2.3). ORM entry is `Model.filter()` (no `db.query`). Cancel has no classmethod/control-plane (§2.7). Enums+exceptions are the only one-class-per-file exemption (§2.3).
