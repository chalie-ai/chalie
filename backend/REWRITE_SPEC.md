...
> - **`skill_builder` self-halt + Ability-base `mp` typing — ✅ DONE** (jul06): the `getattr(self.mp, "request_cancel")(…)` no-op wedge is GONE — the create/edit self-halt now fires the real §2.7 surface `self.mp.turn_execution_service.cancel()` (skill_builder.py:187), and the channel read is typed `self.mp.config.channel` (:168). Root-caused to the base: `Ability.__init__` now types `mp: "MessageProcessor | None"` (via `TYPE_CHECKING`, _ability.py:100) instead of `object | None`, and `_inject_framework_fields` (L243) narrows to typed `self.mp is not None and self.mp.config.SUPPORTS_ASYNC` — closing spec row 340's itemized leak. Boot + typed access verified. **Residual (separate slice, NOT skill_builder):** 5 other abilities still duck-type `getattr(self.mp, "config", …)` — memory.py:148, schedule.py:224/250, web_browse.py:107, web_search.py:104, browser.py:256 — now visible against the typed base; these are the remaining framework-getattr class to close.
...
- **Active-record, builder on the model (Critical 3 / Session ruling; engine ruling 2026-07-07/08).**
  - Reads: `Transcript.filter('channel', ch).order_by('id DESC').limit(30).get()` — predicates are **structured** (`filter(key, value, operator='=')`, `where` alias, `filter_in(key, values)`): the column name is identifier-validated, the operator comes from a fixed whitelist (`=`, `!=`, `<`, `<=`, `>`, `>=`, `LIKE`, `IS`, `IS NOT`), and every value binds as a `?` parameter — a raw predicate string is never accepted. `.filter()/.where()/.filter_in()/.order_by()/.limit()/.offset()` mutate a `Query` and return it; only the terminals hit the DB.
  - **The base owns the full ORM verb space** (owner directive 2026-07-08): `Model.get(pk)`, `.filter()/.where()/.filter_in()/.order_by()/.all()/.first()/.exists()/.count()`; `Query` terminals `get/first/count/exists/pluck(column)/select(*columns)` plus predicate-guarded `update(**fields)`/`delete()`. A per-model hand-roll of anything the base expresses is a defect: delete it and call the inherited verb (`MyModel.count()` — the parent class makes it free).
  - Every model declares `get_table() -> str`, enforced by the `HasTable` Protocol (`contracts/has_table.py`) — the base verbs derive all SQL from it.
  - Writes: `Transcript(channel=ch, content=txt).save()`; `row.delete()`.
  - **Raw-SQL scope ruling:** `cls._bound_connection().execute(...)` inside a **named classmethod on the owning model** is reserved for what the builder genuinely cannot express — joins, OR-across-columns predicates, aggregates (GROUP BY / HAVING / MAX / correlated subselects — e.g. thread-feed summaries like `Transcript.recent_threads(...)`), upserts, and vec0/FTS shadow maintenance. This stays inside I6 (SQL is model-resident). Anything the builder can express uses the builder.
...
> - **✅ RESOLVED (jul06) — document-path data_graph breakage:** the 5 sites no longer import the deleted `get_data_graph_service` / touch `dgs.db`. They now route through a new on-spine active-record model `models/document.py::DocumentRow(DataGraphRow)` (§2.6): reads via `DocumentRow.for_document_id(doc_id).get()` (builder), writes via `set_fragments_active(doc_id, 0|1)` + `purge_by_document_id(doc_id)` inside `Database.transaction()`. `services/data_graph_service` module is fully gone (`ModuleNotFoundError`, 0 prod refs); `abilities/document.py`, `services/document_service.py`, `api/documents.py` all boot clean.
>
...
| `Ability` (+ base framework: `CapabilityAbility`, `AbilityRegistry`, `ToolResult`, `ToolParamError`, `Keys`, `KeyNormalizer`, `KeyHealer`, `BudgetCappedAbility`, `_MCPAbility`, `SearchableAbility`, `ReviewWindowAbility`) | abilities/*.py | **Kept** (Fork scope ruling) — re-point only the spine-touching leaks | `BudgetCappedAbility` bare `ActTrail()` (_budget.py:31) → `self.mp.tool_call_service`; ✅ `Ability.__init__` now types `mp: "MessageProcessor \| None"` (was `object \| None`, _ability.py:100) + `_inject_framework_fields` getattr (L243) → typed `self.mp.config.SUPPORTS_ASYNC` (DONE jul06); ✅ `skill_builder` self-halt wedge → `self.mp.turn_execution_service.cancel()` (DONE). **Residual getattr-mp:** memory.py:148, schedule.py:224/250, web_browse.py:107, web_search.py:104, browser.py:256 |
