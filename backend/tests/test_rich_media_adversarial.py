...
def _tc(result: str) -> dict[str, object]:
...
class TestAdversarialParser:
    def test_id_with_leading_underscore_not_recognised_as_rich(self) -> None:
        ...
        all_text = " ".join(
            cast(str, s.get("content", "")) for s in segs if s["type"] == "text"
        )
    ...
    def test_script_inside_span_synthesis_stripped_before_parse(self) -> None:
        ...
        assert len(rich) == 1
        assert "evil" not in cast(str, rich[0]["synthesis"])
        assert "Partly cloudy." in cast(str, rich[0]["synthesis"])
    ...
    def test_numeric_only_id_not_recognised_as_rich(self) -> None:
