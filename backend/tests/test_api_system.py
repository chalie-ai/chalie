...
class TestSystemAPI:
    ...
    # GET /system/context-usage — per-thread request-size indicator

    @staticmethod
    def _log_call(db: sqlite3.Connection, job_name: str, turn_id: int | None, tokens: int) -> None:
        """Insert one llm_call_log row; created_at defaults, id auto-increments."""
        db.execute(
            "INSERT INTO llm_call_log (job_name, provider, model, tokens_input, turn_id) "
            "VALUES (?, 'p', 'm', ?, ?)",
            (job_name, tokens, turn_id),
        )
        db.commit()

    def test_context_usage_turn_id_narrows_to_that_thread(self, client: FlaskClient, db: sqlite3.Connection) -> None:
        """turn_id scopes the token count to a thread — the latest row for that
        (job, turn) wins, and a same-turn delegate on a different job is ignored."""
        self._log_call(db, 'user:user', 10, 1000)   # thread 10, superseded below
        self._log_call(db, 'user:user', 20, 2000)   # thread 20
        self._log_call(db, 'web_search:user', 20, 50)   # delegate on turn 20, other job
        self._log_call(db, 'user:user', 10, 1500)   # thread 10, latest → wins for turn 10

        with patch('services.provider_cache_service.ProviderCacheService.get_selected_provider',
                   return_value={'max_tokens': 128000}):
            r10 = client.get('/api/system/context-usage?type=user&turn_id=10')
            r20 = client.get('/api/system/context-usage?type=user&turn_id=20')

        assert r10.status_code == 200
        assert r10.get_json()['last_request_tokens'] == 1500
        assert r10.get_json()['context_window'] == 128000
        assert r20.get_json()['last_request_tokens'] == 2000  # delegate's 50 excluded

    def test_context_usage_without_turn_id_returns_latest_overall(self, client: FlaskClient, db: sqlite3.Connection) -> None:
        """Omitting turn_id (footer/spine) returns the job's most recent row across
        all threads — here the last insert, regardless of which thread it belonged to."""
        self._log_call(db, 'user:user', 20, 2000)
        self._log_call(db, 'user:user', 10, 1500)  # most recent overall

        with patch('services.provider_cache_service.ProviderCacheService.get_selected_provider',
                   return_value={'max_tokens': 128000}):
            no_turn = client.get('/api/system/context-usage?type=user')
            neg_one = client.get('/api/system/context-usage?type=user&turn_id=-1')

        assert no_turn.get_json()['last_request_tokens'] == 1500
        assert neg_one.get_json()['last_request_tokens'] == 1500  # -1 is treated as "no turn"

    def test_context_usage_unknown_turn_returns_null_tokens(self, client: FlaskClient, db: sqlite3.Connection) -> None:
        """A thread with no logged calls yields null tokens, not another thread's count."""
        self._log_call(db, 'user:user', 20, 2000)

        with patch('services.provider_cache_service.ProviderCacheService.get_selected_provider',
                   return_value={'max_tokens': 128000}):
            resp = client.get('/api/system/context-usage?type=user&turn_id=999')

        assert resp.status_code == 200
        assert resp.get_json()['last_request_tokens'] is None
