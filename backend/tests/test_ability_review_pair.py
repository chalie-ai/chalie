...
def _seed_tool_call(db: sqlite3.Connection, *, transcript_id: int, tool_name: str, params: str, result: str, created_at: str) -> None:
    db.execute(
...
def _seed_transcript_row(db: sqlite3.Connection, *, channel: str, role: str, content: str, created_at: str) -> None:
    db.execute(
...
def _body_json(rendered: str, tool: str) -> list[dict[str, object]]:
    return cast(list[dict[str, object]], parse_body(rendered, tool))


# ── 1. review_tool_calls happy path: windowed rows, iter ordinals, ok flag ──────


def test_review_tool_calls_happy_path(db: sqlite3.Connection, chat_mp: MP) -> None:
    tid = chat_mp.uid
    _seed_tool_call(
...
def test_review_tool_calls_params_summary_clipped(db: sqlite3.Connection, chat_mp: MP) -> None:
    long_params = '{"q":"' + ("z" * 400) + '"}'
    _seed_tool_call(
    ...
    rows = _body_json(out, "review_tool_calls")
    assert len(rows) == 1
    assert len(cast(str, rows[0]["params_summary"])) <= 120


# ── 3. review_tool_calls empty window → success count=0 with a prose hint ────────


def test_review_tool_calls_empty_window_is_success_hint(db: sqlite3.Connection, chat_mp: MP) -> None:
    out = ToolDispatcher(chat_mp).dispatch(
        "review_tool_calls", {"date_time": _ANCHOR, "act_summary": "x"}
    )
...
def test_review_tool_calls_garbage_date_time_invalid_time(db: sqlite3.Connection, chat_mp: MP) -> None:
    out = ToolDispatcher(chat_mp).dispatch(
...
def test_review_transcript_happy_path(db: sqlite3.Connection, chat_mp: MP) -> None:
...
def test_review_transcript_subagent_toggle(db: sqlite3.Connection, chat_mp: MP) -> None:
    _seed_transcript_row(
        db, channel="user", role="user",
...
def test_review_transcript_buffer_clamp(db: sqlite3.Connection, chat_mp: MP) -> None:
    _seed_transcript_row(
        db, channel="user", role="user",
...
def test_review_transcript_invalid_buffer_is_invalid_param(db: sqlite3.Connection, chat_mp: MP) -> None:
    out = ToolDispatcher(chat_mp).dispatch(
        "review_transcript",
...
def test_review_tool_calls_query_failed_is_loud(db: sqlite3.Connection, chat_mp: MP) -> None:
