...
def test_pre_dispatch_is_noop_by_default():
    """pre_dispatch() on a subclass that does not override it returns None."""

    class _HookAbility(Ability):
        NAME = "hook_ability"
        SUMMARY = "Hook test"
        EXAMPLES = ["a", "b", "c", "d", "e", "f"]
        INPUT_SCHEMA = {}
...
def test_post_dispatch_is_noop_by_default():
    """post_dispatch() on a subclass that does not override it returns None."""

    class _PostHookAbility(Ability):
        NAME = "post_hook_ability"
        SUMMARY = "Post hook test"
        EXAMPLES = ["a", "b", "c", "d", "e", "f"]
        INPUT_SCHEMA = {}
