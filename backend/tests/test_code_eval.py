...
``run()`` returns a ``ToolResult``: success carries the captured output in
``body``; failure is ``status == "error"`` and folds any partial output and the
failure message into ``body`` (the dispatcher renders the ``[code_eval(...)]``
envelope around it). These assert that contract directly.
"""
...
def test_runtime_error_returns_full_traceback():
    ...
    assert result.status == "error", "runtime error must surface as an error result"
    assert "Traceback (most recent call last)" in result.body
    assert "ZeroDivisionError" in result.body
...
def test_runtime_error_preserves_partial_output():
    ...
    # Partial print output is preserved alongside the failure.
    assert "before\n" in result.body


def test_syntax_error_returns_traceback():
    result = _run("print(")
    assert result.status == "error", "syntax error must surface as an error result"
    assert "SyntaxError" in result.body
...
def test_import_is_blocked_and_reported():
    ...
    assert result.status == "error", "blocked import must surface as an error result"
...
def test_long_output_round_trips_through_subprocess():
    ...
    assert result.status == "success"
    assert result.body.strip() == "x" * 200000
