...
def _run(query: object) -> tuple[list[str], dict[str, object], str]:
    """Drive FindToolsAbility.run() against the real production DB on a real
    MessageProcessor. Returns (injected_names, body, rendered_envelope)."""
...
def test_described_action_surfaces_semantic_match() -> None:
    ...
    injected, _body, _r = _run(["add appointment"])
...
def test_implicit_prose_query_surfaces_chalie_docs(query: str) -> None:
    ...
    injected, _body, rendered = _run([query])
...
def test_multi_intent_array_routes_each_entry() -> None:
    ...
    never silent."""
    injected, body, _r = _run(["weather", "send an email", "pizza"])
...
def test_missing_query_is_error_nothing_injected() -> None:
    injected, _body, rendered = _run(None)
...
def test_empty_query_array_is_error() -> None:
    injected, _body, rendered = _run([])
    assert injected == []
    assert "status=error" in rendered
...
def test_semantic_query_degrades_gracefully_when_db_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    injected, body, rendered = _run(["documentation"])
