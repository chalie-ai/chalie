...
2. ``type=chat`` narrows the response to chat rows only;
3. ``type=system`` narrows to system rows only;
...
_SEED_ROWS = (
    # chat, provider-x, alpha -- recent.
    ("chat", "provider-x", "alpha", 100, 20, 10, 2, 5, _RECENT_OFFSET),
    # system, provider-x, alpha -- SAME bucket/model/provider, DIFFERENT type.
    ...
    ("system", "provider-x", "alpha", 40, 8, 4, 1, 2, _RECENT_OFFSET),
    # chat, provider-y, beta -- third producer, third model, still recent.
    ("chat", "provider-y", "beta", 200, 40, 20, 4, 10, _RECENT_OFFSET),
    ...
    (
        "chat", "provider-x", "alpha", 1_000_000, 500_000,
        100_000, 50_000, 25_000, _OUTLIER_OFFSET,
    ),
)

# Hand-computed expectations, derived solely from the seed rows above.
_CHAT_RECENT = 100 + 20 + 10 + 2 + 5 + 200 + 40 + 20 + 4 + 10  # 411
_SYSTEM_RECENT = 40 + 8 + 4 + 1 + 2  # 55
_RECENT_TOTAL = _CHAT_RECENT + _SYSTEM_RECENT  # 466
...
class TestTokenUsageReadPath:
    ...
    def test_entries_grouped_by_bucket_type_model_provider(self, seeded_client) -> None:
        ...
        Counts asserted: tokens_input=100 for chat, 40 for system. Any merge
        would give one big row with summed counts — this assertion fires on
        exactly that.
        """
        ...
        assert len(same_combo) == 2, (
            ...
            f"across chat+system, got {[e['type'] for e in same_combo]}"
        )
        types_seen = sorted({e["type"] for e in same_combo})
        assert types_seen == ["chat", "system"], types_seen

        chat_row = next(e for e in same_combo if e["type"] == "chat")
        sys_row = next(e for e in same_combo if e["type"] == "system")
        assert chat_row["tokens_input"] == 100
        assert chat_row["tokens_output"] == 20
        assert sys_row["tokens_input"] == 40
        assert sys_row["tokens_output"] == 8

    # ------------------------------------------------------------------
    # Claims (b), (c): type filters isolate rows cleanly.
    # ------------------------------------------------------------------

    def test_chat_filter_excludes_system_rows(self, seeded_client) -> None:
        """``type=chat`` must not surface any system entry."""
        resp = seeded_client.get("/api/system/observability/token-usage?window=day&type=chat")
        assert resp.status_code == 200
        body = json.loads(resp.data)

        types_in_response = {e["type"] for e in body["entries"]}
        assert types_in_response <= {"chat"}, (
            f"type=chat still surfaced non-chat entries: {types_in_response}"
        )

    def test_system_filter_excludes_chat_rows(self, seeded_client) -> None:
        """``type=system`` must not surface any chat entry."""
        resp = seeded_client.get("/api/system/observability/token-usage?window=day&type=system")
        assert resp.status_code == 200
        body = json.loads(resp.data)

        types_in_response = {e["type"] for e in body["entries"]}
        assert types_in_response <= {"system"}, (
            f"type=system still surfaced non-system entries: {types_in_response}"
        )

    def test_no_filter_returns_both_types(self, seeded_client) -> None:
        ...
        types_in_response = {e["type"] for e in body["entries"]}
        assert types_in_response >= {"chat", "system"}, (
    ...
    def test_summary_totals_match_seeded_numbers_exact(self, seeded_client) -> None:
        ...
        cache_create+thinking):
          chat/x/alpha   (recent):   100+20+10+2+5   =    137
          system/x/alpha (recent):    40+8+4+1+2     =     55
          chat/y/beta    (recent):  200+40+20+4+10   =    274
          chat/x/alpha   (1yr ago): 1M+500K+100K+50K+25K = 1_675_000

        day_no_filter      = 137+55+274 = 466
        day_chat           = 137+274 = 411
        day_system         = 55
        lifetime_no_filter = 466 + 1_675_000

        A bug merging chat+system into one row would either leave day_system
        inflated or day_chat reduced by the wrong amount; comparing both
        branches independently catches precisely that.
        """
        cases = [
            ("/api/system/observability/token-usage?window=day",
             "day_no_filter", _RECENT_TOTAL),
            ("/api/system/observability/token-usage?window=day&type=chat",
             "day_chat", _CHAT_RECENT),
            ("/api/system/observability/token-usage?window=day&type=system",
             "day_system", _SYSTEM_RECENT),
    ...
    def test_window_bound_excludes_old_row(self, seeded_client) -> None:
