...
class TestTagFormatter:
    ...
    def test_empty_content_omits_body_line(self) -> None:
        result = tag("read")
        assert result == "[read()]\n[end:read]"

    def test_none_content_omits_body_line(self) -> None:
        result = tag("read", cast(str, None))
        assert result == "[read()]\n[end:read]"

    def test_content_with_newlines_is_preserved(self) -> None:
        body = "line one\nline two\nline three"
        result = tag("memory", body, action="recall")
        assert result == f"[memory(action=recall)]\n{body}\n[end:memory]"

    def test_error_only_args_no_body(self) -> None:
    ...
    def test_arg_values_are_str_coerced(self) -> None:
    ...
    def test_name_appears_in_both_opener_and_terminator(self) -> None:
        result = tag("subagent", "body here", sub_id="abc")
        assert result.startswith("[subagent(")
        assert result.endswith("[end:subagent]")

    def test_no_args_produces_empty_parens(self) -> None:
