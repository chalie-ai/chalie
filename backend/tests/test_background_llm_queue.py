...
class TestBackgroundLLMProxy:
    ...
    def test_enqueue_job_pushed_to_queue(self, proxy, fake_store):
    ...
    def test_returns_none_when_queue_full(self, proxy, fake_store):
        ...
        for _ in range(MAX_QUEUE_DEPTH):
            fake_store.rpush(QUEUE_KEY, json.dumps({"job_id": "x"}))

        result = proxy.send_message("sys", "user")
        assert result is None

    def test_returns_none_on_brpop_timeout(self, proxy, fake_store):
        """Proxy should return None when BRPOP times out."""
        with patch.object(fake_store, "brpop", return_value=None):
            result = proxy.send_message("sys", "user")
        assert result is None

    def test_returns_none_on_error_result(self, proxy, fake_store):
    ...
    def test_stale_heartbeat_logs_critical(self, proxy, fake_store, caplog):
        """Proxy should log CRITICAL when worker heartbeat is >30s old."""
        import logging
        stale_ts = str(time.time() - 60)  # 60 seconds ago
        fake_store.set(HEARTBEAT_KEY, stale_ts)
    ...
    def test_job_payload_includes_retry_count_zero(self, proxy, fake_store):
...
class TestGetSleepInterval:
    ...
    def test_normal_when_moderately_recent_interaction(self, fake_store):
        fake_store.set("proactive:default:last_interaction_ts", str(time.time() - 180))
        sleep = _get_sleep_interval(fake_store)
        assert NORMAL_SLEEP * 0.89 <= sleep <= NORMAL_SLEEP * 1.11

    def test_idle_when_no_interaction_key(self, fake_store):
        sleep = _get_sleep_interval(fake_store)
        assert IDLE_SLEEP * 0.89 <= sleep <= IDLE_SLEEP * 1.11

    def test_idle_when_old_interaction(self, fake_store):
        fake_store.set("proactive:default:last_interaction_ts", str(time.time() - 600))
        sleep = _get_sleep_interval(fake_store)
        assert IDLE_SLEEP * 0.89 <= sleep <= IDLE_SLEEP * 1.11

    def test_jitter_applied(self, fake_store):
        """Same state should produce slightly different sleep durations across calls."""
        fake_store.set("proactive:default:last_interaction_ts", str(time.time() - 600))
        sleeps = {_get_sleep_interval(fake_store) for _ in range(20)}
