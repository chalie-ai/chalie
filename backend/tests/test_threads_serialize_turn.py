...
# counter restarts at 1. So two ``image_search`` calls in one turn BOTH carry
# ``image_search_1``. ``serialize_turn`` used to resolve every assistant row's
...
# the first query's images (the reported "keeps rendering the same thing" bug).
# The fix scopes each assistant row's span to its own cycle's tool calls.


def _image_search_rich_result(query: str, image_url: str, ordinal: int) -> str:
    """The on-wire result an ``image_search`` rich card stores: the sealed
    envelope ``[image_search(status=success)]\\n<payload>\\n\\n<instruction>\\n
    [end:image_search]`` whose instruction carries the ordinal-keyed span the
    ...
    instruction = (
        ...
        f"<span id='image_search_{ordinal}'>your synthesis here</span>. "
        "The span renders as a card; without it the user sees only plain text."
    )
    return f"[image_search(status=success)]\n{payload}\n\n{instruction}\n[end:image_search]"


def test_same_tool_rich_cards_in_one_turn_resolve_per_cycle(db: sqlite3.Connection) -> None:
    """Two ``image_search`` calls across two cycles of one turn, both carrying
    ``image_search_1`` (fresh MP per request → ordinal restarts at 1). Before
    ...
    # Cycle 1 — "northern lights" → image_search #1 (ordinal 1).
    ...
    call1 = c1.tool_call_service.start("image_search", {"query": "northern lights"})
    assert call1 is not None  # sanity: real tool-call write
    c1.tool_call_service.finish(
        call1,
        _image_search_rich_result("northern lights", "https://north.example/a.jpg", 1),
        ToolCall.DONE,
    )
    a1 = c1.transcript_service.append_assistant(
        "Here you go: <span id='image_search_1'>Northern lights.</span>"
    )

    # Cycle 2 (same turn) — "southern lights" → image_search #1 AGAIN: the fresh
    ...
    call2 = c2.tool_call_service.start("image_search", {"query": "southern lights"})
    assert call2 is not None  # sanity: real tool-call write
    c2.tool_call_service.finish(
        call2,
        _image_search_rich_result("southern lights", "https://south.example/b.jpg", 1),
        ToolCall.DONE,
    )
    ...
    def rich_query(msg: dict[str, object]) -> str:
