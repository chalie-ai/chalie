...
def _make_provider(db: sqlite3.Connection, *, name: str, max_tokens: int) -> object:
    cur = db.cursor()
    cur.execute(
...
class TestContextUsageEndpoint:
    ...
    def test_returns_last_user_turn_tokens_and_window(self, authed_client: tuple[object, sqlite3.Connection, object]) -> None:
        ...
        ProviderDbService(cast(DatabaseService, _db_mod._shared_db_service)).set_selected_provider(cast(int, pid))
