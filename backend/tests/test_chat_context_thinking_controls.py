...
if TYPE_CHECKING:
    class _FlaskClient:
        def get(self, path: str) -> "_FlaskResponse": ...
...
def _make_provider(db: sqlite3.Connection, *, name: str, max_tokens: int) -> int:
    cur = db.cursor()
    cur.execute(
...
class TestContextUsageEndpoint:
    ...
    def test_returns_last_user_turn_tokens_and_window(self, authed_client: tuple[object, sqlite3.Connection, object]) -> None:
        ...
        ProviderDbService(cast(DatabaseService, _db_mod._shared_db_service)).set_selected_provider(pid)
        ...
        resp = cast("_FlaskClient", client).get('/system/context-usage')
    ...
    def test_delegate_and_thinking_calls_do_not_shadow_user_turn(self, authed_client: tuple[object, sqlite3.Connection, object]) -> None:
        ...
        data = cast("_FlaskClient", client).get('/system/context-usage').get_json()
        # Pinned to the user turn — NOT the newest (delegate) chat-class row.
        assert data['last_request_tokens'] == 17240

    def test_indicator_filter_matches_real_config_jobs(self) -> None:
...
class TestThinkingOverridePersistence:
    ...
    def test_set_then_read_medium(self, db: sqlite3.Connection) -> None:
        from services.database_service import DatabaseService
        from services.settings_service import SettingsService
        from services.thinking_override_service import get_thinking_override
        import services.database_service as _db_mod

        SettingsService(cast(DatabaseService, _db_mod._shared_db_service)).set('thinking_level_override', 'medium')
        assert get_thinking_override() == 'medium'

    def test_set_then_read_high(self, db: sqlite3.Connection) -> None:
        from services.database_service import DatabaseService
        from services.settings_service import SettingsService
        from services.thinking_override_service import get_thinking_override
        import services.database_service as _db_mod

        SettingsService(cast(DatabaseService, _db_mod._shared_db_service)).set('thinking_level_override', 'high')
        assert get_thinking_override() == 'high'

    def test_absent_is_auto_none(self, db: sqlite3.Connection) -> None:
        from services.thinking_override_service import get_thinking_override
        assert get_thinking_override() is None

    def test_invalid_value_treated_as_auto(self, db: sqlite3.Connection) -> None:
        from services.database_service import DatabaseService
        from services.settings_service import SettingsService
        from services.thinking_override_service import get_thinking_override
        import services.database_service as _db_mod

        SettingsService(cast(DatabaseService, _db_mod._shared_db_service)).set('thinking_level_override', 'banana')
        assert get_thinking_override() is None
...
class TestThinkingModePrecedence:
    ...
    def test_gate_level_when_no_override(self) -> None:
        from services.providers import resolve_thinking_mode
...
class TestThinkingGateBypass:
    ...
    def test_override_short_circuits_gate(self, db: sqlite3.Connection) -> None:
    ...
    def test_no_override_leaves_gate_in_control(self, db: sqlite3.Connection) -> None:
