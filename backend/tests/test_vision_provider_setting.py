...
def test_explicit_vision_provider_resolves(db):
    svc = _svc(db)
    ...
    assert status["source"] == "explicit" and status["provider"]["id"] == pid


def test_explicit_nonvision_falls_through_to_none(db):
    svc = _svc(db)
...
def test_auto_default_to_active_vision_provider(db):
    svc = _svc(db)
...
def test_none_when_no_vision_anywhere(db):
    svc = _svc(db)
...
def test_set_none_clears_setting(db):
    svc = _svc(db)
