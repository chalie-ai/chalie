...
class TestDecayEngineService:
    ...
    # ── Real DB: _decay_behavioral_patterns ──────────────────────────

    def test_decay_behavioral_patterns_transitions_overdue_active_to_stale(self, db):
        """_decay_behavioral_patterns flips status active→stale when last_seen is
        older than decay_days, and leaves patterns within their window untouched.

        Seeds:
        - One 'active' pattern with last_seen 200 days ago + decay_days=180  → must become 'stale'.
        - One 'active' pattern with last_seen 5 days ago + decay_days=90     → must stay 'active'.
        """
        import json

        overdue_content = json.dumps({
            "vertical": "meal_times",
            "class": "time_routine",
            "slots": {"event_label": "dinner", "day_bucket": "fri", "hour_window": "18:30-20:00"},
            "recurrence": 7,
            "sigma_confidence": 4.8,
            "first_seen": "2025-10-01T00:00:00+00:00",
            "last_seen": "2025-10-07T00:00:00+00:00",  # ~200 days ago relative to 2026-04-25
            "evidence_super_ep_ids": [],
            "status": "active",
            "source": "llm",
            "decay_days": 180,
        })
        fresh_content = json.dumps({
            "vertical": "meetings",
            "class": "event_cadence",
            "slots": {"event_type": "standup", "cadence_days": 1, "next_expected": "2026-04-26"},
            "recurrence": 3,
            "sigma_confidence": 2.1,
            "first_seen": "2026-04-20T00:00:00+00:00",
            "last_seen": "2026-04-20T00:00:00+00:00",  # 5 days ago, well within 90
            "evidence_super_ep_ids": [],
            "status": "active",
            "source": "llm",
            "decay_days": 90,
        ...
        db.execute(
            ...
            "VALUES ('behavioral_pattern', 'meal_times:dinner:fri', ?, 1)",
            (overdue_content,),
        )
        db.execute(
            "INSERT INTO data_graph (kind, key, value, active) "
            "VALUES ('behavioral_pattern', 'meetings:standup', ?, 1)",
            (fresh_content,),
        )
        db.commit()
        ...
        count = svc._decay_behavioral_patterns()
        ...
        overdue_row = conn.execute(
            "SELECT value FROM data_graph WHERE key='meal_times:dinner:fri'"
        ).fetchone()
        fresh_row = conn.execute(
            "SELECT value FROM data_graph WHERE key='meetings:standup'"
        ).fetchone()

        assert overdue_row is not None
        assert json.loads(overdue_row[0])['status'] == 'stale'

        assert fresh_row is not None
        assert json.loads(fresh_row[0])['status'] == 'active'
