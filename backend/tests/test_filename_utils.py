...
class TestSafeFilename:
    ...
    def test_strips_null_and_control_chars(self) -> None:
    ...
    def test_unsafe_only_name_returns_empty(self) -> None:
        # Caller is responsible for substituting a fallback.
        assert safe_filename('..') == ''
        assert safe_filename('') == ''

    def test_caps_length_at_255_preserving_extension(self) -> None:
        result = safe_filename('a' * 300 + '.txt')
        assert len(result) == 255
        assert result.endswith('.txt')

    def test_preserves_normal_name_and_extension(self) -> None:
        assert safe_filename('report.tar.gz') == 'report.tar.gz'

    def test_caps_when_extension_itself_exceeds_limit(self) -> None:
