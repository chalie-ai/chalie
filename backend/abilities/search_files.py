...
class SearchFilesAbility(Ability[SearchFilesParamsBag]):
    ...
    def _search_content(self, query: str, page: int) -> ToolResult:
        ...
        return SearchFilesAbility._paginate("glob", paths, page, False)
    ...
    def _search(self, action: str, query: str, directory: str, page: int, context_lines: int) -> ToolResult:
        ...
        try:
            results, truncated = SearchFilesAbility._gather(action, query, root, context_lines)
        ...
        return SearchFilesAbility._paginate(action, results, page, truncated)
    ...
    def _gather(action: str, query: str, root: Path, context_lines: int) -> tuple[list[object], bool]:
        ...
        """
        path = SearchFilesAbility._cache_path(action, query, context_lines, root)
        try:
        ...
        if action == "glob":
            results, truncated = SearchFilesAbility._do_glob(root, query, deadline)
        else:
            results, truncated = SearchFilesAbility._do_grep(root, query, context_lines, deadline)
    ...
    def _iter_files(root: Path, deadline: float) -> Generator[str, None, None]:
        ...
        for dirpath, dirnames, filenames in os.walk(root):
            dirnames[:] = [d for d in dirnames if not SearchFilesAbility._skip(os.path.join(dirpath, d))]
            if SearchFilesAbility._skip(dirpath):
                continue
    ...
    def _do_glob(root: Path, pattern: str, deadline: float) -> tuple[list[object], bool]:
        ...
        *root*, capped at ``_MAX_RESULTS``.
        ...
        truncated = False
        for fp in SearchFilesAbility._iter_files(root, deadline):
            if recursive:
    ...
    def _do_grep(
        ...
        truncated = False
        for fp in SearchFilesAbility._iter_files(root, deadline):
            rows = SearchFilesAbility._grep_file(fp, pattern, context_lines, _MAX_RESULTS - total)
            if not rows:
                continue
            try:
                mtime = os.path.getmtime(fp)
                ...
                for i, raw in enumerate(fh):
                    ...
                    for m in open_rows:
                        ...
                        if len(cast("list[str]", m["_after"])) >= context_lines:
                            rows.append(SearchFilesAbility._finalize(m, context_lines))
                        else:
                            still_open.append(m)
