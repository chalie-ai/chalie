...
if TYPE_CHECKING:
    from services.mcp_client_service import McpClientService as _McpSvc
...
class McpManagerAbility(Ability):
    ...
    @staticmethod
    def _server_row(svc: "_McpSvc", server: dict[str, object]) -> dict[str, object]:
        return {
            "id": server["id"],
            "name": server["name"],
            "url": server["host"],
            ...
            "tool_count": len(svc.get_server_tools(cast(str, server["id"]))),
        }

    @staticmethod
    def _resolve(svc: "_McpSvc", params: dict[str, object]) -> "tuple[str, str] | ToolResult":
