...
class WeatherAbility(Ability[WeatherParamsBag]):
    ...
    def run(self, params: WeatherParamsBag) -> ToolResult:
        ...
        lat, lon, location_name = WeatherAbility._extract_location(self.telemetry)
        ...
        cache_key = WeatherAbility._build_cache_key(location_param, lat, lon, location_name)

        cached = WeatherAbility._get_fresh_cache(cache_key)
        if cached is not None:
            return ToolResult.ok(cached, rich=cached)

        result, open_meteo_err, wttr_err = WeatherAbility._fetch_with_fallback(location_param, lat, lon, location_name, cache_key)

        if result is not None:
            WeatherAbility._cache[cache_key] = (result, time.time())
            return ToolResult.ok(result, rich=result)

        payload = WeatherAbility._stale_or_error(cache_key, open_meteo_err, wttr_err)
        if "error" in payload:
            return ToolResult.err(
    ...
    def _fetch_with_fallback(location_param: str, lat: float | None, lon: float | None, location_name: str | None, cache_key: str) -> tuple[dict[str, object] | None, str, str]:
        ...
        use_telemetry_coords = have_coords and (not location_param or WeatherAbility._matches_telemetry(location_param, location_name))

        if use_telemetry_coords:
            result, open_meteo_err = WeatherAbility._fetch_open_meteo(cast(float, lat), cast(float, lon), location_name or f"{cast(float, lat):.4f}, {cast(float, lon):.4f}")

        if result is None and location_param:
            result, wttr_err = WeatherAbility._fetch_wttr(location_param)
        ...
        if result is None:
            ...
            if use_telemetry_coords:
                result, _ = WeatherAbility._fetch_open_meteo(cast(float, lat), cast(float, lon), location_name or f"{cast(float, lat):.4f}, {cast(float, lon):.4f}", timeout=5)
            if result is None and location_param:
                result, _ = WeatherAbility._fetch_wttr(location_param, timeout=5)
                if result is not None and use_telemetry_coords and location_name:
                    result["location"] = location_name
    ...
    def _fetch_open_meteo(lat: float, lon: float, location_name: str, timeout: int = 15) -> tuple[dict[str, object] | None, str]:
        try:
            ...
            hourly_strip = WeatherAbility._extract_hourly_strip(data.get("hourly", {}), cc.get("time", ""))
            ...
            return {
                ...
                "wind_direction": WeatherAbility._degrees_to_compass(float(cc.get("wind_direction_10m", 0))),
                "visibility_km": None,
                "uv_index": None,
    ...
    def _fetch_wttr(location: str, timeout: int = 15) -> tuple[dict[str, object] | None, str]:
        try:
            ...
            return {
                ...
                "is_raining": any(w in condition_lower for w in _RAIN_WORDS),
                "is_daylight": WeatherAbility._estimate_daylight(obs_time),
                "is_hot": feels_like_c >= 30,
                "is_cold": feels_like_c <= 10,
