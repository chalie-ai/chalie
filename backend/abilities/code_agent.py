...
class CodeAgentAbility(DelegateAbility):
    ...
    def run(self, params: dict[str, object]) -> ToolResult:
        ...
        text = _append_handoff_footer(agent_mp.result(), agent_mp)
...
def _append_handoff_footer(text: str, agent_mp: "MessageProcessor") -> str:
...
def _footer_lines(agent_mp: "MessageProcessor") -> list[str]:
    ...
    """
    mutations = _collect_mutations(agent_mp)
    return [_render_line(abs_path, status) for abs_path, status in mutations.items()]


def _collect_mutations(agent_mp: "MessageProcessor") -> dict[str, str]:
    ...
    for call in agent_mp.tool_call_service.by_turn():
        if call.state != ToolCall.DONE or call.tool_name not in _MUTATION_TOOLS:
            continue
        try:
        ...
        if tool == "manage_files":
            _handle_manage_files(mutations, call_params)
...
def _handle_manage_files(
    ...
    if action == "create":
        _mark(mutations, call_params.get(Keys.path), _STATUS_CREATED)
    elif action == "delete":
        _mark(mutations, call_params.get(Keys.path), _STATUS_DELETED)
...
def _handle_file_write(
    ...
    ``modified``."""
    body = _envelope_body(rendered_result, "file_write")
    try:
        ...
        status = _STATUS_MODIFIED
    _mark(mutations, call_params.get(Keys.path), status)


def _handle_move(mutations: dict[str, str], call_params: dict[str, object]) -> None:
    ...
    with .ts."""
    _apply_move(mutations, call_params.get(Keys.current_path), call_params.get(Keys.new_path))


def _mark(mutations: dict[str, str], raw_path: object, status: str) -> None:
    ...
    if not isinstance(raw_path, str) or not raw_path.endswith(".ts"):
        return
    if status == _STATUS_MODIFIED and mutations.get(raw_path) == _STATUS_CREATED:
        return
    mutations[raw_path] = status


def _apply_move(mutations: dict[str, str], raw_source: object, raw_destination: object) -> None:
    ...
    if isinstance(raw_destination, str) and raw_destination.endswith(".ts"):
        mutations[raw_destination] = _STATUS_MOVED
...
def _envelope_body(rendered_result: str, tool_name: str) -> str:
...
def _render_line(abs_path: str, status: str) -> str:
