...
def _handle_edit(params: dict[str, object]) -> ToolResult:
    ...
    try:
        ...
        skill_id = cast(int, existing["id"])
        updated_meta: dict[str, str | int] = {
            "title": title,
            "use_for": cast(str, params.get(Keys.use_for) or "").strip() or cast(str, existing["use_for"]),
            "content": cast(str, params.get(Keys.content) or "").strip() or cast(str, existing["content"]),
            "tags": cast(str, params.get(Keys.tags) or "").strip() if params.get(Keys.tags) is not None else cast(str, existing["tags"] or ""),
