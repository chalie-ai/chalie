...
class Ability(ABC):
    ...
    # ── Trail statics (implemented in T4) ─────────────────────────────────────
    ...
    def record(
        ...
        """Write one raw trail row to tool_calls.

        The only write path for the trail — no rendering at write time.
        Full implementation in T4.  Until T4, delegates to the legacy
        ToolRenderAndRecordService so existing trail consumers keep working.
        ...
        try:
            ...
            logger.debug("[Ability.record] trail write failed (non-fatal): %s", exc)
