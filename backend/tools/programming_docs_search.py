...
# Tags that signal the main content region (tried in order)
...
class _TextExtractor(HTMLParser):
    """Extract visible text from HTML, stripping noise tags."""
...
def _fetch(url):
    """Fetch a URL, return (status_code, body_str) or raise."""
...
def _head_ok(url, timeout=5):
    """Return True if HEAD request returns 2xx/3xx."""
    try:
...
def _extract_text(html):
    """Extract clean text from HTML, targeting the main content region."""
    body = html
...
def _fetch_and_extract(url):
    """Fetch a URL and return extracted text. Never raises."""
    try:
        _, html = _fetch(url)
...
# ---------------------------------------------------------------------------
# Search result parser helpers
# ---------------------------------------------------------------------------

def _parse_links(html, base_url):
    """Extract (url, title) pairs from <a> tags in an HTML fragment."""
    results = []
...
def _sphinx_search(base_url, query, max_results=5):
    """Search a Sphinx-hosted docs site via its server-side search."""
    try:
        url = f"{base_url.rstrip('/')}/search/?q={urllib.parse.quote(query)}"
        _, html = _fetch(url)
        results = []
        # Sphinx search results are in <li> tags within a search-results list
...
class _Source:
    """Base class for a documentation source."""
    name = ""
    id = ""
    aliases = []
    base_url = ""
    ...
    def search(self, query, max_results=5):
        """Return list of {url, title} dicts. Never raises."""
        return []
...
class PHPSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        # Fallback: php.net quickref search
        try:
...
class LaravelSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        # Laravel docs have predictable URL patterns
    ...
    def search(self, query, max_results=5):
        ...
        if _head_ok(f"https://docs.python.org/3/library/{module}.html"):
            candidates.append({
                ...
                "title": f"{module} — Python docs",
            })

        # Fallback: genindex lookup
        if not candidates:
            try:
...
class DjangoSource(_Source):
    ...
    def search(self, query, max_results=5):
        # Django has a server-side search with JSON suggestions API
        try:
            url = f"https://docs.djangoproject.com/en/5.0/search/?q={urllib.parse.quote(query)}"
            _, html = _fetch(url)
            results = []
            # Django search results: <dt> with link, <dd> with snippet
            for m in re.finditer(
                r'<dt>\s*<a\s[^>]*href=["\']([^"\']+)["\'][^>]*>(.*?)</a>',
                html, re.DOTALL | re.IGNORECASE,
            ):
...
class NumPySource(_Source):
    ...
    def search(self, query, max_results=5):
        q = query.strip().lower().replace(" ", ".")
        # NumPy has predictable reference URLs: numpy.org/doc/stable/reference/generated/numpy.{func}.html
        url = f"https://numpy.org/doc/stable/reference/generated/numpy.{q}.html"
        if _head_ok(url):
            return [{"url": url, "title": f"numpy.{q}"}]

        # Try without numpy prefix
...
class PandasSource(_Source):
    ...
    def search(self, query, max_results=5):
        q = query.strip()
        # Pandas API: pandas.pydata.org/docs/reference/api/pandas.{name}.html
    ...
    def search(self, query, max_results=5):
        try:
            ...
            for doc in data.get("documents", []):
                results.append({
                    "url": f"https://developer.mozilla.org{doc['mdn_url']}",
                    "title": doc.get("title", ""),
                })
            return results[:max_results]
        except Exception:
...
class NodeSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        # Node.js API docs: nodejs.org/api/{module}.html
...
class ReactSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        # React hooks and APIs have predictable URLs
        # Hooks: react.dev/reference/react/useState
        # Components: react.dev/reference/react/Component
...
class RustSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        # Strategy 1: direct std lib URL patterns
        q_lower = q.lower().replace("::", "/")
        std_patterns = [
        ...
        # Strategy 2: docs.rs crate search (covers tokio, serde, actix etc.)
        try:
        ...
        if not results:
            results.append({
                "url": f"https://doc.rust-lang.org/std/?search={urllib.parse.quote(query)}",
                "title": f"Rust std: {query}",
            })
...
class JavaSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        if not results:
            results.append({
                "url": f"https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/{q}.html",
                "title": f"Java docs: {q}",
            })
...
class SpringSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        if class_name[0:1].isupper():
            results.append({
                "url": f"https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/{q.replace('.', '/')}.html",
                "title": f"Spring API: {class_name}",
            })
...
class RubySource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        results.append({
            "url": f"https://ruby-doc.org/search.html?q={urllib.parse.quote(query)}",
            "title": f"Search Ruby docs for: {query}",
        })
        return results[:max_results]
...
class RailsSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        # Rails API: api.rubyonrails.org/classes/{Class}.html
        class_name = q.split(".")[0].split("#")[0].split("::")[0].strip()
        if class_name[0:1].isupper():
            # Try with ActiveRecord/ActionController etc. namespacing
...
class DartSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        results.append({
            "url": f"https://dart.dev/search?q={urllib.parse.quote(query)}",
            "title": f"Search Dart docs for: {query}",
        })
        return results[:max_results]
...
class FlutterSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        # Flutter API: api.flutter.dev/flutter/{library}/{Class}-class.html
        ...
        # Try as a widget (most common lookup)
...
class BashSource(_Source):
    ...
    def search(self, query, max_results=5):
        q = query.strip().lower().replace(" ", "-")
        return [{
            "url": f"https://www.gnu.org/software/bash/manual/bash.html#{urllib.parse.quote(q)}",
            "title": f"Bash manual: {query}",
        }, {
...
class SQLSource(_Source):
    ...
    def search(self, query, max_results=5):
        ...
        if not results:
            results.append({
                "url": f"https://www.sqlite.org/lang_{query.lower().replace(' ', '')}.html",
                "title": f"SQLite: {query}",
            })
...
def lookup(language, query):
    """
    Search official docs for a language/framework and return extracted content.

    Returns dict with: text, url, source (on success) or error (on failure).
    """
    ...
    if not results:
        return {
            "error": f"No results found for '{query}' in {source.name} docs.",
            "source": source.name,
            "url": source.base_url,
        }
