...
def execute(args: dict, ctx: dict) -> dict:
    ...
    if not isinstance(evidence, list) or len(evidence) < 2:
        return {"error": "insufficient_evidence", "required_min": 2, "got": len(evidence) if isinstance(evidence, list) else 0}
    ...
    with db.connection() as conn:
        ...
        if existing:
            ...
            cur = conn.execute(
                ...
                "VALUES (?, ?, ?, ?, ?, ?, 1)",
                ("behavioral_pattern", name, json.dumps(new_value), now_iso, now_iso, "pattern_match"),
            )
