...
class SearchEnricher:
    """Best-effort enrichment of search results from page meta tags."""
    ...
    def enrich_missing_summaries(results: list[dict[str, object]]) -> list[dict[str, object]]:
        ...
        if len(need_enrich) == 1:
            idx, r = need_enrich[0]
            enriched_by_index[idx] = SearchEnricher._enrich_one(r)
        else:
            with ThreadPoolExecutor(max_workers=_MAX_WORKERS) as executor:
                future_to_idx = {
                    executor.submit(SearchEnricher._enrich_one, r): idx
