...
class SearchFetcher:
    """Fetches search results from multiple providers with circuit breaking."""

    _MAX_WORKERS: ClassVar[int] = 3
    _DDG_TIME_RANGE_MAP: ClassVar[dict[str, str]] = {
        "day": "d", "week": "w", "month": "m", "year": "y"
    }

    # ── Circuit Breaker ─────────────────────────────────────────────────────

    _breakers: ClassVar[dict[str, dict[str, object]]] = {}
    _breaker_lock: ClassVar[threading.RLock] = threading.RLock()
    _BREAKER_THRESHOLD: ClassVar[int] = 3
    _BREAKER_RECOVERY_S: ClassVar[float] = 60.0

    @classmethod
    def _get_breaker(cls, name: str) -> dict[str, object]:
    ...
    @classmethod
    def _is_breaker_open(cls, name: str) -> bool:
        """Check if a provider's circuit breaker is open (tripped)."""
        with cls._breaker_lock:
            b = cls._get_breaker(name)
            if b['state'] == 'closed':
                return False
            if b['opened_at'] and (time.time() - cast("float", b['opened_at'])) >= cls._BREAKER_RECOVERY_S:
    ...
    def _record_failure(cls, name: str) -> None:
        ...
        with cls._breaker_lock:
            ...
            if cast("int", b['consecutive_failures']) >= cls._BREAKER_THRESHOLD:
                b['state'] = 'open'
    ...
    _rate_lock: ClassVar[threading.RLock] = threading.RLock()

    @classmethod
    def _enforce_rate_limit(cls, provider: dict[str, object]) -> None:
        ...
        with cls._rate_lock:
            last = cls._last_call_times.get(name, 0)
            elapsed = time.time() - last
            if elapsed < delay:
                time.sleep(delay - elapsed)
            cls._last_call_times[name] = time.time()
    ...
    def _fetch_one(provider: dict[str, object], query: str, limit: int) -> list[dict[str, object]]:
        ...
        if SearchFetcher._is_breaker_open(name):
            logger.info('[SEARCH] skipping %s — circuit breaker open', name)
            return []

        SearchFetcher._enforce_rate_limit(provider)
        ...
        try:
            ...
            results = SearchTransformer.transform(name, fmt, raw_data, limit)

            SearchFetcher._record_success(name)
            ...
            latency_ms = int((time.time() - t0) * 1000)
            SearchFetcher._record_failure(name)
            logger.warning(
    ...
    @classmethod
    def fetch_providers(cls, providers: list[dict[str, object]], query: str, limit: int = 5) -> list[dict[str, object]]:
        """
        ...
        if len(providers) == 1:
            return cls._fetch_one(providers[0], query, limit)

        all_results: list[dict[str, object]] = []

        with ThreadPoolExecutor(max_workers=cls._MAX_WORKERS) as executor:
            futures = {
                executor.submit(cls._fetch_one, p, query, limit): cast("str", p['name'])
                for p in providers
            }
    ...
    @classmethod
    def fetch_ddg_fallback(cls, query: str, limit: int = 5) -> list[dict[str, object]]:
        ...
        try:
            ...
            return cls._transform_ddg_results(raw)
