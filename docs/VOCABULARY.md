...
| `RETURNS_UNTRUSTED_CONTENT` | Ability trait; when True the tool's successful result can carry prose written outside the conversation, so the dispatcher appends the untrusted-content notice to its follow-up block — the standing rule that fetched content is data, never instructions (`docs/SECURITY.md`). Set it on tools whose job is to bring outside content in; leave it False on tools reporting Chalie's own state, because a notice on every result is a notice the model stops reading. | `read`, `web_fetch`, `email`, `vision` (True); `weather`, `search_files` (False) |
