...
**Innate abilities (ALWAYS_AVAILABLE)** are pre-injected on every ACT iteration for the current processor. For `UserMessageProcessor` this set is: `document`, `find_tools`, `list`, `memory`, `read`, `review_tool_calls`, `review_transcript`, `schedule`, `timer`. `ScheduledPromptProcessor` inherits UMP's full tool surface (same ALWAYS_AVAILABLE + DISCOVERABLE) — it is a UMP with hidden input and a clean context window. Other processors declare their own lists; the subconscious `PatternMatchProcessor`, for example, carries only `save_pattern` and `save_graph`. `SubagentProcessor` sets `ALWAYS_AVAILABLE` per-instance from the type registry (`SUBAGENT_TYPES[agent_type]['native_tools']`). `DMNMessageProcessor` deliberately excludes `timer` because it has no user-channel render surface — the rich-media card would never reach the UI.

**Discoverable abilities (DISCOVERABLE)** are never pre-injected. The `find_tools` ability performs semantic search against `abilities.sqlite` at runtime, restricted to the names listed in the calling processor's `DISCOVERABLE` class var. For `UserMessageProcessor` the discoverable set is: `browser`, `calendar`, `code_eval`, `contacts`, `email`, `news`, `programming_docs_search`, `search`, `subagent`, `weather`. When the LLM invokes `find_tools`, the matching abilities become available for the remainder of that ACT loop. All external (first-party + interface) abilities are reachable exclusively through this path — pre-injecting them would bloat context, create staleness bugs, and break tool-agnostic routing.
