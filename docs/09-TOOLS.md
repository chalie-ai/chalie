...
**Innate abilities (ALWAYS_AVAILABLE)** are pre-injected on every ACT iteration for the current processor. For `UserMessageProcessor` this set is: `document`, `find_tools`, `list`, `memory`, `read`, `review_tool_calls`, `schedule`, `subagent`, `timer`. Other processors declare their own lists; the subconscious `PatternMatchProcessor`, for example, carries only `save_pattern` and `save_graph`. `SubagentProcessor` sets `ALWAYS_AVAILABLE` per-instance from the type registry (`SUBAGENT_TYPES[agent_type]['native_tools']`). `DMNMessageProcessor` and `ScheduledMessageProcessor` deliberately exclude `timer` because they have no user-channel render surface — the rich-media card would never reach the UI.
