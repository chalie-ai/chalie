...
**Innate abilities (ALWAYS_AVAILABLE)** are pre-injected on every ACT iteration for the current processor. For `UserMessageProcessor` this set is: `document`, `find_tools`, `list`, `memory`, `read`, `review_tool_calls`, `review_transcript`, `schedule`, `timer`. Scheduled prompts and external agent HITL flows create fresh UMP instances via `dispatch_message()` with `hidden_input=True` — they inherit the full UMP tool surface. Other processors declare their own lists; the subconscious `PatternMatchProcessor`, for example, carries only `save_pattern` and `save_graph`. `SubagentProcessor` sets `ALWAYS_AVAILABLE` per-instance from the type registry (`SUBAGENT_TYPES[agent_type]['native_tools']`). `DMNMessageProcessor` deliberately excludes `timer` because it has no user-channel render surface — the rich-media card would never reach the UI.
