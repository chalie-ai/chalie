...
**The gap, closed.** One deletion left a real hole, and it was closed the way the ruling requires. The test that bought back the every-controller-is-mounted guarantee was checking something true: a controller class the route table forgets is reachable at no URL, and the symptom in production is a 404 that reads like a frontend bug. So the check moved into the module that owns the table — importing it walks the two controller packages, diffs the classes it finds against the table, and raises naming each one that has no path. Two details are the whole difference between a guard and a second test. It lives beside the table rather than in a suite, so it fires on any import, in any process, before an app exists; and it matches classes on where they are *defined*, so a re-export cannot accuse an innocent module and the shared bases are never candidates. Its other half needed no code at all: the framework already refuses to boot when two controllers collide on one generated name, which the deleted test's own docstring had said out loud. Before writing a guard, check what the framework already guarantees — and prove the guard can fire, because an enforcement that cannot trigger is the same lie as a test that cannot go red, with a boot-time cost added.
