...
**Speaker (text-to-speech)** — a speaker icon appears below each Chalie message. Clicking it opens a centered overlay player with play/pause, seek ±10 s, a progress bar, and a close button. Only one message plays at a time; opening a new one cancels the previous. Playback streams: `/voice/synthesize` returns an `application/x-ndjson` body — one JSON line per sentence-sized WAV chunk (`{index, total, audio}`) followed by a `{done: true, total}` success sentinel. If a sentence fails mid-stream the server emits `{error, total, index}` (without `done`); the player aborts the fetch and surfaces the error rather than spinning forever. Sentences are produced by `pysbd` segmentation; pause length between chunks is keyed off terminal punctuation (`.`/`!`/`?` → 0.28–0.30 s, `:`/`;` → 0.20 s, `,` → 0.14 s) and baked into each WAV as trailing silence. Text preprocessing before synthesis: HTML pre-pass via `extract_plaintext`, then `markdown-it-py` render → `extract_plaintext` again, bare URLs rewritten to their spoken host (`http://google.com/123` → `google dot com`), `num2words` ordinal expansion, ALL-CAPS short-acronym letter-spacing, whitespace collapse. Synthesis runs sequentially in the request thread under a single TTS lock, so the shared Kokoro ONNX session never sees concurrent inference. VoicePlayer reads the response body via `ReadableStreamDefaultReader`, decodes each chunk into an `AudioBuffer` and chains playback via `AudioBufferSourceNode.onended`, so audio starts as soon as the first chunk is ready. `AudioContext.resume()` runs synchronously inside the click handler, satisfying iOS Safari's autoplay policy.
