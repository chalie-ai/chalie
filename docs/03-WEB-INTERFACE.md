...
**Speaker (text-to-speech)** — a speaker icon appears below each Chalie message. Clicking it opens a centered overlay player with play/pause, seek ±10 s, a progress bar, and a close button. Only one message plays at a time; opening a new one cancels the previous. Playback streams: `/voice/synthesize` returns an `application/x-ndjson` body — one JSON line per sentence-sized WAV chunk (`{index, total, audio}`) followed by a `{done: true, total}` sentinel. Synthesis runs sequentially in the request thread under a single TTS lock, so the shared Kokoro ONNX session never sees concurrent inference. VoicePlayer reads the response body via `ReadableStreamDefaultReader`, decodes each chunk into an `AudioBuffer` and chains playback via `AudioBufferSourceNode.onended`, so audio starts as soon as the first chunk is ready. If the stream closes without the sentinel (server crash mid-synthesis) the player surfaces an error rather than silently treating partial audio as the full message. `AudioContext.resume()` runs synchronously inside the click handler, satisfying iOS Safari's autoplay policy.
