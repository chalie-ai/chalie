...
│   ├── voice_recorder.js  # Voice input (mic → MediaRecorder → WAV → STT → transcript paste)
│   ├── voice_player.js    # Voice output (speaker button → overlay audio player with iOS unlock)
...
**Asset freshness**: Every HTML page is served through `_serve_versioned_html` (`backend/api/__init__.py`), which rewrites each same-origin `<script src>`, `<link href>`, `<img src>`, `<source src>` at response time to inject `-{VERSION}` before the extension — e.g. `app.js` becomes `app-0.3.3.js`, `/shared/theme.css` becomes `/shared/theme-0.3.3.css`. The version string is read from the top-level `VERSION` file at module load. Static-file routes (`/shared/<path>`, `/brain/<path>`, `/<path>`, etc.) transparently strip the `-{VERSION}` suffix before looking the file up on disk, so nothing is renamed on disk. Versioned filenames are used instead of query strings because some Service Workers and intermediate proxies ignore query strings when keying their cache, whereas a distinct filename is universally treated as a new resource. HTML responses themselves are sent `Cache-Control: no-cache, no-store, must-revalidate` so the document can never be cached with stale versioned URLs. Intra-module ES `import`s aren't rewritten — they rely on Flask's `SEND_FILE_MAX_AGE_DEFAULT = 0` + ETag revalidation (304 when unchanged) to stay fresh. `sw.js` is exempt from rewriting (browsers identify Service Worker registrations by script URL; renaming would orphan the current registration). The Service Worker does **not** precache assets; it only hosts the share-target redirect and Web Push handlers, and its `activate` step wipes any legacy caches left by earlier versions.
...
#### Response Generation
- **`personality/personality_service.py`** — Loads the 3,125-row `voices.jsonl` corpus (keyed by 5-tuple `(warmth, mood, expressiveness, curiosity, humor)`, steps ∈ {-2,-1,0,1,2}) once on first call via a double-checked lock. Exposes `get_voice(tup)`, `get_current_tuple()`, `set_current_tuple()`, `get_current_voice()`. The active tuple is persisted via `SettingsService` (`personality.tuple` key) and defaults to neutral `(0,0,0,0,0)` when unset. `UserMessageProcessor.getSystemPrompt()` prepends `When responding; {voice}` as the first line of the assembled system prompt; DMN, goal-pursuit, scheduled, and encoder processors do not include it.
