...
- **Delegate tools** — `web_search` and `web_browse` each run a focused ACT turn on a `delegate:<name>` channel (`MessageProcessor.process(goal, config)`, `max_iterations=50`, 600 s deadline). On the user channel they run async (daemon thread, immediate ack, synthesis delivered back via `dispatch_message(..., hidden_input=True)` — the single chat chokepoint in `api/chat.py`); on any non-user channel they run synchronously inline. They replace the former `subagent` ability and its `SubagentProcessor`. See the "Delegate tools" section below.
...
**Shared config shape** (encoded in each subclass's `__init__`): `channel=f"delegate:{NAME}"`, `role=NAME`, `policy_channel` inherited from the caller via `policy_channel_for(channel)` (user→CHAT, `external-agent:*`→EXTERNAL_AGENT, else SUBCONSCIOUS), `max_iterations=50`, `discoverable=[]`, `blocked=frozenset()`, plus `skip_transcript=True`, `skip_input_row=True`, `suppress_history=True`, `broadcast_to=None`, `memory_seed=False` — a delegate is a scratch turn, not a conversation.

**Recursion guard.** A delegate carries no `find_tools` and `discoverable=[]`, so it can only call the fixed tools in its own `always_available` surface — and no delegate tool is in that surface. A delegate therefore can never spawn another delegate; the guard is *structural*, so the former `build_blocked()` / `DELEGATE_TOOL_NAMES` helpers (and per-delegate `blocked` sets) were removed. `_delegate.py` retains `DELEGATE_DEADLINE_SECONDS = 600` (wall-clock horizon), `policy_channel_for(channel)` (caller→policy-channel map), `delegate_goal(params)` (normalises `goal`/`query`), and `render_trail(mp)` (renders the caller's act-trail into the delegate's user prompt).
...
- **Sync (called on any non-user channel):** `dispatch()` runs the delegate inline via `_run_with_timeout(...)` and blocks the parent iteration until it returns.
