...
5. **Post-turn fan-out** — services that react to a completed turn (save suggestion detection, metrics) run after the atomic write. The response is already on its way to the client before fan-out begins. (Pre-v0.6.0 also reset a DMN idle-timer here; DMN is now driven entirely by the subconscious worker tick.)
...
### Concrete abilities (22 dispatchable)

`abilities/{bash, browser, calendar, chalie_docs, code_eval, contacts, document, email, find_skills, find_tools, home, list, memory, news, place, programming_docs_search, read, review_tool_calls, schedule, search, skill_builder, subagent, weather}.py`. `abilities.sqlite` indexes all 23. `abilities_sha.json` mirrors the per-row sha and is checked in CI.
