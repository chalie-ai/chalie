...
2. **ACT loop** — assembles one user message (conversation history as literal text, world state, the input, the accumulated tool trail) and calls the LLM. Tool calls are dispatched and their results fed back; the loop repeats until the model answers in plain text or the cancel event fires — there is no iteration cap. If the request outgrows the context window, compaction fires transparently inside the loop and the request is rebuilt.
3. **Record** — writes the assistant transcript row, then runs `config.post_turn_hooks` in a failure-isolated loop.
 
On the user channel, tool activity is streamed live via WebSocket events (`act_tool_start`, `act_tool_end`), and each tool-bearing step emits an interim `message` frame (flagged `interim`) as it settles; the final `message`/`done` events are sent by the caller once `process()` returns. The broker fans **every** frame — the `user_message` echo broadcast when the message first arrives, `status`, the live ACT events, and the interim/final `message`s — out to every open surface, so both sides of the conversation stay in sync everywhere. The live ACT trail nonetheless appears only on the surface that started the turn: idle peers drop content-free ACT frames client-side (a render-policy choice), not because the server withholds them.
...
The **subconscious worker** ticks every 5 minutes but only fires when the user has been idle for 30+ minutes and there is something new since the last run. Each tick runs nine steps, each isolated so one failure can't block the rest:
...
9. **Proactive research** — at most once every six hours (its own durable clock, checked each tick), a grounded pass over the user summary and the latest conversation summary researches the web and news for things the user would care about; genuinely interesting findings are saved as `discovery` memories that decay after about two weeks, to resurface in a later conversation — nothing is pushed to chat
