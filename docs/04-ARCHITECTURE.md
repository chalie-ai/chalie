...
Compaction is just another flat turn. **Two universal configs** — `CompactionConfig` (history) and `TrailHandoverConfig` (trail) — serve every channel (delegates included); there is no per-channel or per-subagent compaction config. Both kinds are driven by the single `MessageProcessor._compact()` method, fired from `_loop` when a context-overflow is caught (and proactively when the un-compacted history grows large). They differ in input, output row, and system prompt: **history compaction** summarises the older `## Previous Messages` block via `MessageProcessor.process(prev, CompactionConfig())` and writes the summary to the `transcript` table as a *durable* `role='compaction'` row — **that row's own `id` becomes the watermark**, so `_previous_rows()` (`id > watermark`) naturally excludes it and everything before it; **trail compaction** summarises this turn's act-trail via `TrailHandoverConfig` and records an *ephemeral* `tool_name='trail_compaction'` row in `tool_calls`. There is no recursion-guard channel set: both configs carry `suppress_history=True` and expose no tools, so `_previous_rows()` returns `[]` and `_compact()` is a no-op inside a compaction turn. `compaction_persistence.get_compaction(channel)` reads the newest `transcript` `role='compaction'` row — there is no success/failure status and no separate `compactions` table. History compaction uses `ContinuityCompactionSystemPrompt`; trail compaction uses its own concise handover prompt (`trail_handover.py`).
...
| **Compaction** | LLM-generated continuity summaries: **history** as durable `transcript` rows with `role='compaction'` (the row's id is the watermark); **trail** as ephemeral `tool_calls` rows with `tool_name='trail_compaction'` | No |
