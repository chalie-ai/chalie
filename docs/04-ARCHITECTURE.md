...
3. **ACT loop** — the processor assembles a single user message containing the literal conversation history, world state, memory seed, and the current input, then calls the LLM. If the LLM invokes a tool, the result is appended to the trail and the loop continues. This repeats until the model returns a plain text response, a cooperative cancel signal is received (`_cancel_event`), or the processor's iteration cap is reached. User-facing processors (UMP, SubagentProcessor) have no iteration cap — they run until the model finishes or the user stops them via `POST /chat/interrupt` (or `POST /chat/subagent/<sub_id>/stop` for subagents). When a turn is cancelled, `_cleanup_cancelled_turn()` deletes all tool_call and transcript rows for that turn so no trace persists. Background processors retain hard caps (DMN=100, EAMP=200, PatternMatch=100, GeoPattern=100).
...
1. `SubagentAbility.execute()` registers the subagent in a module-level `_active_subagents` registry (keyed by `sub_id`) and spawns a daemon thread. A `subagent_start` WS event is broadcast so the frontend can show the subagent in the task drawer.
...
3. The daemon calls `dispatch_message(envelope, source='subagent', hidden_input=True)` — the single chat chokepoint in `api/chat.py`, which always starts a fresh UMP turn. The raw envelope never appears in the user transcript — only the synthesized assistant response does.
4. In the `finally` block the daemon unregisters from `_active_subagents` and broadcasts a `subagent_end` WS event (status: `done`, `cancelled`, or `error`).

The user can cancel a running subagent via `POST /chat/subagent/<sub_id>/stop`, which sets the subagent's `_cancel_event`. The `sub_id` is stored in `tool_calls.params` for each dispatched subagent.
...
The `execute(channel, params, telemetry)` method is the sole dispatch surface. `pre_dispatch(params) -> dict` is called by `ActDispatcherService` before policy enforcement — abilities override it to normalise or escalate parameters (e.g. `BashAbility` uses it to upgrade the LLM's self-classification via heuristic inspection). The default is a pass-through.
...
### Concrete abilities (23 dispatchable)

`abilities/{bash, browser, calendar, chalie_docs, code_eval, contacts, document, email, find_skills, find_tools, git, home, list, memory, news, place, programming_docs_search, read, review_tool_calls, schedule, search, skill_builder, subagent, weather}.py`. `abilities.sqlite` indexes all 24. `abilities_sha.json` mirrors the per-row sha and is checked in CI.
...
- `git` — read/write GitHub and GitLab repositories. **Hybrid architecture**: git CLI (`subprocess`, arg-list, no shell) handles `clone`, `diff`, `commit`, and `push`; raw `requests` handles the REST-only actions (`get_repo`, `list_branches`, `list_commits`, `get_commit`, `list_prs`, `get_pr`, `list_issues`, `get_issue`, `list_releases`, `search_repos`, `read_file`, `create_pr`, `merge_pr`). 17 actions total (13 READ, 4 WRITE). Works without a token (public repos, read-only); with a Personal Access Token unlocks private repos and write actions. Self-hosted GitHub Enterprise or GitLab supported via the `host` credential key; custom hosts are validated through the shared `abilities/_ssrf.py` `is_private_url()` guard and must use HTTPS. Tokens are never written to `.git/config` or process args — passed exclusively via the subprocess environment (`GIT_TOKEN`) and redacted from all error strings before reaching the LLM. Workspace lifecycle: `clone` creates a `chalie_git_*` temp dir (shallow `--depth 1` by default); the LLM edits files there using existing file tools, then calls `commit`/`push`; stale workspaces older than 6 h are removed at the start of each `clone`. Force-push and pushes to default/protected branches (`main`, `master`, `develop`, `trunk`) are unconditionally blocked. Delegates to `GitCapability` (`capabilities/git_capability/`). Configured via Brain → Capabilities. Policy: all 13 READ actions = `allow` in every channel; WRITE actions (`commit`, `push`, `create_pr`, `merge_pr`) = `ask` in chat and subagent, `deny` in subconscious and external_agent. TIMEOUT = 120 (clone/push can be slow on large repos). DISCOVERABLE only.
...
- `subagent` — spawns a daemon thread that runs `SubagentProcessor`. On completion, calls `dispatch_message(envelope, source='subagent', hidden_input=True)` — the single chat chokepoint, which always starts a fresh UMP turn. The raw subagent envelope is never written to the user transcript (only the synthesized assistant response is).
