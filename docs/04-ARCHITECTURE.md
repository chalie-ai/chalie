...
**Turn-zero seeding.** There is no `pre_act()`/`send()` hook — both were removed with the subclasses. Instead, `_setup()` calls `_seed_turn_zero()` after the input transcript row is written (so `self._uid` is populated) but before iteration 0. It performs two declarative, config-gated behaviours, each via `ToolDispatcher(self).dispatch()` so the call blocks, records a durable `tool_calls` row, and is rendered into the trail exactly like an LLM-issued call: (a) when `config.memory_seed` is set (the user channel), one `memory` `recall` keyed on the raw input — passed with `_auto=True`, which marks it as the silent background seed: it retrieves at the 30%-narrower `SEED_RADIUS_BASELINE` (0.35 vs the explicit `RECALL_RADIUS_BASELINE` 0.5) for a tighter, higher-precision set and carries **no** fallback guardrail. (Explicit, model-invoked recalls no longer silently fan out to `document.search`/`schedule.search` either — that hidden delegation was removed in TKT-878; instead they append a one-line guardrail steering the model to those search tools on its own judgement.); (b) one `document` `upload` per file in `metadata["attachments"]` (upload is the ingest — no second auto `view`), fanned out across a bounded `ThreadPoolExecutor` and joined at the `with`-block barrier so the now network-bound vision/OCR extractions overlap instead of serialising while every upload still completes before turn 0 (TKT-838 — thread-safe via the single-threaded document write queue, thread-local WAL connections, and random-hex `doc_id`s). The model's first turn already sees the memory matches and uploaded documents in its trail — they are not injected into the user prompt.
