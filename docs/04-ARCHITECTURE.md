...
- `memory` — 8 radius constants promoted to `ClassVar` (`RECALL_RADIUS_BASELINE`, `SEED_RADIUS_BASELINE`, etc.) so tests can patch them by name. Module-level `recall_episodes()` function preserved for importability by the turn-0 seed path (`_seed_turn_zero`).
