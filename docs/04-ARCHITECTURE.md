...
`abilities/{bash, browser, calendar, chalie_docs, code_eval, contacts, document, email, file_permissions, file_write, find_skills, find_tools, home, list, mcp_manager, memory, news, place, programming_docs_search, read, review_tool_calls, review_transcript, save_graph, save_pattern, schedule, search, search_files, skill_builder, skill_manager, thinking, timer, ubiquiti, weather, web_browse, web_download, web_search}.py`. `abilities.sqlite` indexes all registered abilities **except `thinking`** (internal-dispatch-only; excluded by `build_ability_db.py`); `abilities_sha.json` mirrors the per-row sha for the 35 indexed abilities and is checked in CI. (`save_pattern` / `save_graph` are `SYSTEM` abilities reachable only by the pattern-match and geo-pattern passes — see below.)
...
- `read` — URL fetch via the shared `services/web_fetch.py` stack (`fetch_text(profile=BROWSER)`); `_BLOCKED_PATH_PREFIXES`, `_URL_FETCH_TIMEOUT` as `ClassVar`. SSRF guard is the single source `services/ssrf.py` (explicit pre-check for the styled private-url error, plus web_fetch's own backstop).
- `web_download` — streamed file download from URL to `/tmp/chalie_downloads/` (default) or explicit destination via `services/web_fetch.py` (`stream_to_file(profile=DOWNLOAD)`). SSRF guard via the single source `services/ssrf.py`, scheme validation (http/https only), blocked destination prefixes, 100 MB size cap, retry on `ConnectionError`, SSL verify-then-fallback. Policy: chat=ask, external_agent=deny.
...
Image attachments are handled by the document pipeline: `text_extractor._extract_image()` routes through the shared `abilities.vision.describe_image()` core, which forks once on whether a vision provider is configured — provider set → a single-shot `MessageProcessor` on the vision provider produces a rich description (`RICH_INDEX_PROMPT`); none → local RapidOCR via `image_context_service.analyze()`. The returned description is what gets embedded + FTS5-indexed, so an uploaded image becomes searchable by its visual content. The no-vision user-facing note is intentionally NOT indexed (index noise). A configured-but-failing vision provider raises rather than being swallowed — `extract_text` can therefore propagate a provider error on the upload path. The upload pipeline (`api.documents._run_upload_extraction`) is image-aware on the empty-text branch: a textless image (no vision provider → empty OCR) is persisted `ready` with `chunk_count=0` — viewable and re-queryable via the vision tool, NEVER marked `failed`; only non-image empty extractions fail. The `document(action='upload')` success result carries both `id=` and `hash=` (the SHA-256 of the uploaded bytes) so a follow-up `view`/dedup can key on the content hash. The `POST /documents/upload` REST endpoint is the Brain document-management UI's surface: it is now a **thin synchronous wrapper** that stages its multipart upload to a temp path, calls the same `ingest_file()` core, and returns the **terminal** status (`ready`/`failed`) plus duplicate matches — both the chat and library surfaces share the one path-based ingest.
