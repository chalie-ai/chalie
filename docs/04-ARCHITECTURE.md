...
1. **Setup & turn-zero seed** — `_setup()` writes the input transcript row (capturing the uid) and runs the deliberation gate: a lightweight ONNX classifier assigns a continuous deliberation score (0.0–1.0) on the row; higher scores nudge the prompt toward more careful reasoning and very high scores trigger a one-shot pre-reasoning exploration pass before the tool loop begins. It then calls `_seed_turn_zero()`, which (user channel) dispatches a `memory` recall keyed on the input and one `document` upload per attachment — each through `Ability.use()`, so the matches and uploaded files land in the turn-0 trail rather than being injected into the prompt.
...
WebSocket frame
  └─ daemon thread
       └─ MessageProcessor.process(raw_input, make_user_config(…))
            ...
            │             memory recall + document upload via Ability.use)
            ├─ ACT loop ──────────────────────────────────────────┐
...
- **`Ability.use()` is the single dispatch chokepoint.** Every ACT-loop tool call — user channel, DMN, delegates, action buttons, even the framework's turn-0 seed calls — routes through this one static method (`match → PolicyManager.wrap(execute) → record`). Tool errors return structured strings to the LLM; they never surface to the user or crash the loop.
...
**Delivery is channel-decided, not type-decided.** Every delegate sets `ASYNC_CAPABLE = True`, but `Ability.use()` takes the async path only when the *dispatching* channel supports it — `_supports_async_delivery(channel)` is `True` only for `channel == "user"`:
...
- **Sync (called on any non-user channel):** `dispatch()` runs the delegate inline via `_run_ability(...)` — synchronous, no wall-clock bound — and blocks the parent iteration until it returns.
...
Each ability implements `run(self, params)` — the sole per-ability execution surface. The base `Ability.execute(params, act_summary)` orchestrates the call (invoked by `Ability.use()` after policy enforcement); the dispatch spine `_run_ability()` sets `self.telemetry` immediately before calling `run()`. **Abilities run to completion — there is no framework execution timeout or watchdog.** The one exception is `code_eval`, which runs its sandbox in a `spawn` subprocess with its own hard 10-minute cap (the only way to force-kill arbitrary CPU-bound code — a thread cannot be interrupted). `pre_dispatch(params) -> dict` is called by `Ability.use()` before policy enforcement — abilities override it to normalise or escalate parameters (e.g. `BashAbility` uses it to upgrade the LLM's self-classification via heuristic inspection). The default is a pass-through. `ASYNC_CAPABLE: ClassVar[bool] = False` opts an ability into async delivery on user-channel calls (daemon thread, immediate ack, result delivered via `dispatch_message` on completion).
...
Upload *is* the ingest: there is no second auto `document(action='view')` call. Because the seed call routes through `Ability.use()`, it records a `tool_calls` row rendered into the turn-0 trail, so the model's first turn already sees the uploaded document.

Both calls go through `Ability.use()` — policy enforcement, WS tool events, and `tool_calls` audit rows are generated naturally. The results land in the ACT trail so the LLM sees file content on iter-0 of the ACT loop.
