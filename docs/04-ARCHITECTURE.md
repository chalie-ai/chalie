...
**Turn-zero seeding.** There is no `pre_act()`/`send()` hook — both were removed with the subclasses. Instead, `_setup()` calls `_seed_turn_zero()` after the input transcript row is written (so `self._uid` is populated) but before iteration 0. It performs two declarative, config-gated behaviours, each via `ToolDispatcher(self).dispatch()` so the call blocks, records a durable `tool_calls` row, and is rendered into the trail exactly like an LLM-issued call: (a) when `config.memory_seed` is set (the user channel), one `memory` `recall` keyed on the raw input — passed with `_auto=True` so the background seed recall does **not** fan out to `document.search` + `schedule.search` (that delegation is reserved for explicit, model-invoked recalls); (b) one `document` `upload` per file in `metadata["attachments"]` (upload is the ingest — no second auto `view`), fanned out across a bounded `ThreadPoolExecutor` and joined at the `with`-block barrier so the now network-bound vision/OCR extractions overlap instead of serialising while every upload still completes before turn 0 (TKT-838 — thread-safe via the single-threaded document write queue, thread-local WAL connections, and random-hex `doc_id`s). The model's first turn already sees the memory matches and uploaded documents in its trail — they are not injected into the user prompt.
