...
Upload *is* the ingest: there is no second auto `document(action='view')` call. Because the seed call routes through `ToolDispatcher.dispatch()`, it records a `tool_calls` row rendered into the turn-0 trail, so the model's first turn already sees the uploaded document.
...
Image attachments are handled by the document pipeline: `text_extractor._extract_image()` routes through the shared `abilities.vision.describe_image()` core (TKT-838), which forks once on whether a vision provider is configured — provider set → a single-shot `MessageProcessor` on the vision provider produces a rich description (`RICH_INDEX_PROMPT`); none → local RapidOCR via `image_context_service.analyze()`. The returned description is what gets embedded + FTS5-indexed, so an uploaded image becomes searchable by its visual content. The no-vision user-facing note is intentionally NOT indexed (index noise). A configured-but-failing vision provider raises rather than being swallowed — `extract_text` can therefore propagate a provider error on the upload path. The upload pipeline (`api.documents._run_upload_extraction`) is image-aware on the empty-text branch: a textless image (no vision provider → empty OCR) is persisted `ready` with `chunk_count=0` — viewable and re-queryable via the vision tool, NEVER marked `failed`; only non-image empty extractions fail. The `document(action='upload')` success result carries both `id=` and `hash=` (the SHA-256 of the uploaded bytes) so a follow-up `view`/dedup can key on the content hash. The `/documents/upload` REST endpoint remains available for the Brain document-management UI.
