...
- `memory` — a **thin adapter**: it owns only the tool metadata and the `run()` action dispatch. The store/recall/reflect/forget handlers, the episode recall engine with its dynamic radius (`recall_episodes()`), data-graph search, reflection layer expansion, response formatting and recall telemetry live in `services/memory_retrieval.py`, so the same engine is reachable from non-ability callers (e.g. the `/api/updates/memory` REST endpoint, which imports `handle_store` from the service) without importing an ability. The 8 radius tuning constants are the service's source of truth; `MemoryAbility` re-exposes them as `ClassVar`s (`RECALL_RADIUS_BASELINE`, `SEED_RADIUS_BASELINE`, etc.) so they stay inspectable and mechanically tunable. The turn-0 seed path still reaches recall the normal way — `_seed_turn_zero` dispatches `memory(action='recall', _auto=True)` through `ToolDispatcher`, not via a direct import.
