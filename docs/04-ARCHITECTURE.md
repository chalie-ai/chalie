...
On the user channel, tool activity is streamed live to the browser via WebSocket events (`act_tool_start`, `act_tool_end`); the final `message`/`done` events are sent by the caller once `process()` returns.
