...
`UserMessageProcessor` sets `ALWAYS_AVAILABLE` to the nine core cognitive abilities (`document`, `find_tools`, `list`, `memory`, `read`, `review_tool_calls`, `review_transcript`, `schedule`, `timer`) and `DISCOVERABLE` to thirteen abilities (`browser`, `calendar`, `code_eval`, `contacts`, `email`, `home`, `news`, `place`, `programming_docs_search`, `search`, `subagent`, `ubiquiti`, `weather`). Scheduled prompts and external agent HITL flows create fresh UMP instances via `dispatch_message()` with `hidden_input=True` — the UMP reads this from metadata and sets `SKIP_INPUT_ROW=True` so the raw trigger never enters the user transcript while the synthesized response does. `calendar`, `contacts`, `email`, `home`, and `ubiquiti` are capability-backed personal-data/infrastructure tools — DISCOVERABLE in UMP and DMN but excluded from `SubagentProcessor` (headless processor that should not access personal data without explicit user intent). Discord is a communication channel, not a tool — see `ExternalChatMessageProcessor` above. Other processors narrow both lists to match their scope — `PatternMatchProcessor`, for example, sets `ALWAYS_AVAILABLE = ["save_pattern", "save_graph"]` and `DISCOVERABLE = []`. `SubagentProcessor` sets `ALWAYS_AVAILABLE` per-instance from `SUBAGENT_TYPES[agent_type]['native_tools']` and `DISCOVERABLE` to the full external set minus a blocklist (`subagent`, `save_graph`, `detect_pattern`) and capability tools (`email`, `calendar`, `contacts`).
...
`abilities/{browser, calendar, code_eval, contacts, document, email, find_tools, home, list, memory, news, place, programming_docs_search, read, review_tool_calls, schedule, search, subagent, weather}.py`. `abilities.sqlite` indexes all 24 (19 dispatchable + `review_transcript`, `save_pattern`, `save_graph`, `steer`, `timer`). `abilities_sha.json` mirrors the per-row sha and is checked in CI. Discord is NOT an ability — it is a communication channel handled by `ExternalChatMessageProcessor`.
