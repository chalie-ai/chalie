...
Compaction is just another flat turn, fired through the **normal tool-dispatch chokepoint**. `MessageProcessor._dispatch_compaction()` dispatches two INTERNAL, never-discoverable abilities — `tool_chain_compactor` (trail) and `chat_history_compactor` (history) — via `ToolDispatcher.dispatch()`, the same machinery as the turn-0 `memory`/`thinking` seeds; there is no `_compact()` method and no inline compaction path. Going through dispatch means each compactor auto-records its `tool_calls` row and auto-emits the act-trail WebSocket events, so compaction surfaces in the frontend act-trail with no hand-rolled emit. Compaction relies **solely on context-window size** — there is no turn-count trigger. `Providers._fit_request()` trims the oldest rendered history rows until the scaffolded request leaves `max(10% of the window, 8k tokens)` of response headroom (looping on `mp.drop_oldest_previous_message()`, which is guaranteed to terminate when the history is exhausted); each drop sets `mp._compaction_pending`, and after the send `_loop` calls `_dispatch_compaction()` to fold the dropped backlog into the checkpoint. A provider that rejects the payload outright (`PayloadTooLargeError`, 413) triggers a **one-shot** `_dispatch_compaction()` (guarded by `mp._payload_compacted`). There is no `ContextOverflowError` and no retry counter. The two abilities differ in input, output, and system prompt: **history compaction** summarises the older `## Previous Messages` block via `MessageProcessor.process(combined, ChatHistoryCompactionConfig())` and writes the model's output **verbatim** to the `transcript` table as a *durable* `role='compaction'` row — **that row's own `id` becomes the watermark**, so `_previous_rows()` (`id > watermark`) naturally excludes it and everything before it; **trail compaction** summarises this turn's act-trail via `ToolChainCompactionConfig` and records its handover as a `tool_name='tool_chain_compactor'` row in `tool_calls` that becomes the new trail boundary. Both configs set `thinking_mode = "high"`, carry `suppress_history=True`, expose no tools, and use `channel="compaction"` (a dedicated name so the inner send's own checkpoint lookup is empty). `compaction_persistence.get_compaction(channel)` reads the newest `transcript` `role='compaction'` row — there is no parser (`<analysis>`/`<summary>` tags are gone), no success/failure status, and no separate `compactions` table. History compaction uses `ChatHistoryCompactionSystemPrompt`; trail compaction uses `ToolChainCompactionSystemPrompt`.
