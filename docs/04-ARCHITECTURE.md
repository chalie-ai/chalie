...
`ExternalAgentMessageProcessor` (`CHANNEL='external-agent:{name}'`, `ROLE='user'`, `USAGE_CLASS='external_agent'`) handles agent-to-agent turns arriving via the MCP server. Each external agent gets its own dynamic channel string, isolating its conversation history. `ExternalChatMessageProcessor` (`CHANNEL='discord'`, `ROLE='third-party-user'` or `'user'` for the owner, `USAGE_CLASS='discord'`) handles inbound Discord messages under the `discord` policy context. It uses `ExternalChatSystemMessagePrompt`, which instructs the model in plain-text/Discord-markdown output and enforces public-channel privacy rules. When the message author's Discord user ID matches the owner's configured ID, `ROLE` is promoted to `'user'` (owner trust level). Both processors bypass `dispatch_message()` — they are not user-channel turns.
...
### Concrete abilities (19 dispatchable)

`abilities/{browser, calendar, code_eval, contacts, discord, document, email, find_tools, home, list, memory, news, place, programming_docs_search, read, review_tool_calls, schedule, search, subagent, weather}.py`. `abilities.sqlite` indexes all 20. `abilities_sha.json` mirrors the per-row sha and is checked in CI.
...
- `discord` — delegates to `DiscordCapability`'s tool handler closures. Actions: `send_message`, `read_messages`, `list_channels`, `list_servers`. The capability runs a `DiscordBotRunner` daemon thread with its own asyncio event loop for the discord.py Gateway connection. Inbound messages from Discord enter the processing pipeline via `dispatch_message(source='discord')`. Auto-replies are sent back to the originating Discord channel after each UMP turn completes. Server and channel access is gated by `allowed_guilds`/`allowed_channels` credential entries; a custom system prompt can be stored as a credential and prepended to each inbound message context. Read actions (`read_messages`, `list_channels`, `list_servers`) are `allow`-gated in chat; `send_message` is `ask`-gated in chat and `deny`-gated for external agents. DISCOVERABLE in UMP only.
...
- `ubiquiti` — delegates to `UbiquitiCapability`'s tool handler closures. Actions: `list_devices`, `list_clients`, `get_info`, `control_client`, `control_device`, `manage_wlan`, `manage_port_forward`, `manage_traffic_rule`, `authorize_guest`. Communicates with UniFi OS controllers via REST API — site endpoints under `/proxy/network/api/s/{site}/` and v2 traffic rules under `/v2/api/site/{site}/trafficrules`. Dual auth: API key (`X-API-KEY` header) or username/password session (`POST /api/auth/login` with CSRF token). Module-level `_session_cache` keyed by URL for session reuse; automatic 401 re-login. SSL fallback convention (try verify=True, catch SSLError, retry verify=False). Read actions (`list_devices`, `list_clients`, `get_info`) are `allow`-gated in chat/subconscious/external_agent; write actions (`control_client`, `control_device`, `manage_wlan`, `manage_port_forward`, `manage_traffic_rule`, `authorize_guest`) are `ask`-gated in chat and `deny`-gated for subconscious/external_agent. DISCOVERABLE in UMP + DMN only.
...
**Policy enforcement.** `ActDispatcherService.dispatch_action()` calls `PolicyService.check(action_id, context)` between handler lookup and execution. Three states: `allow` (proceed), `ask` (block dispatch thread and surface a WebSocket `permission_request` to the chat UI — 30 s timeout, auto-deny), `deny` (reject immediately). Five independent contexts derived from `MessageProcessor.USAGE_CLASS`: `chat`, `subagent`, `subconscious`, `external_agent`, `discord`. In subconscious context, `ask` auto-rejects because no user is present; the rejection is logged to `policy_blocked_log`. The `discord` context mirrors `external_agent` defaults except `discord.send_message` is `allow` (auto-reply is safe for inbound Discord turns). Action IDs are derived from `Ability.NAME` + `INPUT_SCHEMA.properties.action.enum` (53 action IDs across 22 abilities). Defaults are seeded on startup via `PolicyService.seed_defaults()` (`INSERT OR IGNORE`). The enforcement is fail-open: any exception in `_enforce_policy` returns `None` (allow). Unknown actions (not in the default matrix) skip enforcement entirely. Schema: `policy_rules(action_id, context, state)` + `policy_blocked_log`. API: `GET/PUT /api/policies`, `GET/DELETE /api/policies/blocked`, `POST /api/policies/reset`. Brain exposes a Policies tab with per-context grouped toggles, presets (Careful/Balanced/Autonomous), and a blocked-actions audit log.
...
| **Subconscious worker** | 5-minute tick that runs latent cognition (super-episode consolidation, decay, pattern match, user-summary synthesis, DMN, capability sync, geo pattern extraction) only when the user has been idle for ≥ 30 minutes. Capability sync calls `monitor()` on each connected capability — the scheduler only stores calendar event data. |
