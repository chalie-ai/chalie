...
- `find_skills` — inherits `SearchableAbility`. RRF (vec + FTS5) discovery against `abilities/assets/skills.sqlite`. Returns curated step-by-step tool-calling playbooks sourced from 5 YAML files in `backend/abilities/skills/` (competitor analysis, meal planning, performance review, fitness routine, technology evaluation). Each result is annotated with personalisation rules from the `skill_associations` table when available. Falls back to FTS-only search when `EmbeddingService` fails. ALWAYS_AVAILABLE on all user-facing processors.
