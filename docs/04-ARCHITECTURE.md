...
`abilities/{browser, calendar, code_eval, contacts, document, email, find_skills, find_tools, home, list, memory, news, place, programming_docs_search, read, review_tool_calls, schedule, search, subagent, weather}.py`. `abilities.sqlite` indexes all 20. `abilities_sha.json` mirrors the per-row sha and is checked in CI.
...
- `find_skills` — RRF (vec + FTS5) discovery against `abilities/assets/skills.sqlite`. Returns curated step-by-step playbooks sourced from YAML files in `backend/abilities/skills/`. Each result is annotated with personalisation rules from the `skill_associations` table when available. Falls back to FTS-only search when `EmbeddingService` fails. ALWAYS_AVAILABLE on all user-facing processors.
