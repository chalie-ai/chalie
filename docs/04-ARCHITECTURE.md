...
**Asset freshness**: Every HTML page is served through `_serve_versioned_html` (`backend/api/__init__.py`), which rewrites each same-origin `<script src>`, `<link href>`, `<img src>`, `<source src>` at response time to append `?v=<file_mtime>`. Touch an asset → mtime changes → URL changes → every cache layer (browser, proxy, future CDN) is forced to fetch fresh bytes. HTML responses themselves are sent `Cache-Control: no-cache, no-store, must-revalidate` so the document can never be cached with stale `?v=` tokens. Intra-module ES `import`s aren't rewritten — they rely on Flask's `SEND_FILE_MAX_AGE_DEFAULT = 0` + ETag revalidation (304 when unchanged) to stay fresh. The Service Worker does **not** precache assets; it only hosts the share-target redirect and Web Push handlers, and its `activate` step wipes any legacy caches left by earlier versions.
