...
### Concrete abilities (20 dispatchable)

`abilities/{browser, calendar, chalie_docs, code_eval, contacts, document, email, find_skills, find_tools, home, list, memory, news, place, programming_docs_search, read, review_tool_calls, schedule, search, subagent, weather}.py`. `abilities.sqlite` indexes all 21. `abilities_sha.json` mirrors the per-row sha and is checked in CI.
