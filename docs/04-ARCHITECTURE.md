...
- `calendar` — delegates to `MailCapability`'s CalDAV handler closures. Actions: `list_events`, `get_event`, `update_event`. DISCOVERABLE in UMP + DMN only. Subconscious worker syncs every 10 min via `AbilityRegistry.get("calendar").execute()`.
...
- `contacts` — delegates to `MailCapability`'s CardDAV handler closures. Actions: `list`, `get`. DISCOVERABLE in UMP + DMN only. Subconscious worker syncs every 60 min.
