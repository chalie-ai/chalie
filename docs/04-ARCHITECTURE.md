...
- `read` — `requests` at module top; `_BROWSER_HEADERS`, `_BLOCKED_PATH_PREFIXES`, `_URL_FETCH_TIMEOUT` as `ClassVar`. SSRF guard delegated to the single source `services/ssrf.py`.
- `web_download` — streaming file download from URL to `/tmp/chalie_downloads/` (default) or explicit destination. SSRF guard via the single source `services/ssrf.py`, scheme validation (http/https only), blocked destination prefixes, 100 MB size cap, retry on `ConnectionError`, SSL verify-then-fallback. Policy: chat=ask, external_agent=deny.
