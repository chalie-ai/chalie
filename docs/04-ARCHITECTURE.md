...
- **`user_message_processor.py`** — `UserMessageProcessor(MessageProcessor)` subclass for the user channel. `CHANNEL='user'`, `ROLE='user'`, `NATIVE_TOOLS=ALL_SKILL_NAMES`, `MAX_ITERATIONS=30`, `MAX_TIMEOUT=900s`. Constructor adds `on_narration: Callable[[str, int], None] | None`. `_run_memory_seed()` auto-seeds episodes once per turn (`caller='seed'`, durable `ephemeral=0` DTO). `_run_thinking_gate()` (base-level, user channel only) classifies deliberation depth after memory seed and before the ACT loop — see `thinking_level_classifier_service.py`. When `level='high'`, fires a one-shot exploration pass (same-job LLM call, `tools=[]`) and stores the result as a durable `tool_calls` row (`name='thinking'`, `ephemeral=0`); `_wrap_with_exploration()` prepends it to the user_body on every ACT iteration; excluded from user-facing render by `_NEVER_RENDER_IN_PREVIOUS`. `getUserPrompt()` assembles: world state, system awareness, `## Previous Messages`, memory seed line, current turn, ACT trail. `getSystemPrompt()` weaves `{{voice_modulation}}` and `{{adaptive_directives}}` into the unified template — no prompt tail mutation for thinking level (deliberation depth is communicated via the provider-native `thinking_mode` flag on every API call). `getTools()` excludes `rich_render` for voice-mode turns. `postTurn()` runs 8-service fan-out (see `docs/13-MESSAGE-FLOW.md` §4). Personal facts are captured inline by the LLM-native memory skill (`kind='trait'`); `_check_trait_contradiction` fires synchronously inside `memory_skill._handle_store()` — no background trait pipeline.
- **`onnx_inference_service.py`** — Shared gte-modernbert encoder + swappable 2-layer MLP classifier heads (v0.9.0). Borrows the encoder session + tokenizer from `embedding_service` (loaded once per process, ~596 MB); each task head is a tiny numpy MLP (~800 KB) loaded from a per-task `.npz` file at first use. Boot-time sha256 pin: `classifier_meta.json::base_encoder_sha256` is verified against the on-disk encoder ONNX via streaming hash; mismatch raises `RuntimeError` and the task is NOT registered. On success emits `[CLASSIFIER BOOT] <task> sha256=… input_dim=… hidden_dim=… num_classes=… activation=…`. `MODEL_REGISTRY` is the single source of truth for downloadable tasks (`contradiction`, `thinking_level`); `ensure_models()` fetches `<prefix>-classifier_meta.json` + `<prefix>_head.npz` from the `chalie-ai/models` `v0.9.0` release. `predict(task, text, extra_features=None)` returns `(label, confidence)` — label strings come directly from the head (e.g. `low`/`medium`/`high`, `temporal_change`/`true_contradiction`/…). Weights follow torch `nn.Linear` `(out, in)` layout — forward uses `features @ W1.T + b1`. **Boot-gate visibility:** `_failed_registrations` records `(task, error)` for any task that fails sha256 verification; `ready` returns False whenever the list is non-empty (so `/api/system` fails fast instead of masking a degraded classifier); `degraded` is True when registration completed but at least one task failed. The system endpoint surfaces `status='degraded'` with the failed task list — silent fallback in production is no longer possible.
- **`thinking_level_classifier_service.py`** — Pre-ACT deliberation-depth gate (user channel only). `classify(user_turn, prev_level, channel='') -> {'level': str, 'confidence': float, 'fallback': bool}`. Level ∈ `{low, medium, high}`. Raw user turn goes straight into the encoder — no prompt template. `prev_level` is encoded as a 4-dim one-hot (`PREV_LEVEL_TO_IDX = {none:0, low:1, medium:2, high:3}`, FROZEN into the trained head's W1) concatenated to the 768-d pooled embedding → 772-d MLP features. Confidence threshold 0.70; below threshold applies sticky fallback (prev_level or `'low'` cold-start, `fallback=True`). Anti-demotion guard: when the head emits `low` while `prev_level ∈ {medium, high}`, the gate inherits the previous level so a single ambiguous follow-up ("yes go on") cannot collapse to chit-chat. **Chain-break:** a module-level `_INHERIT_COUNT_BY_CHANNEL` dict keyed by channel tracks consecutive inherits; on the second consecutive low-head signal the gate yields to the head and demotes — prevents the permanent `high` lock-in failure mode where a channel that ever hit `high` could never escape. Counter resets on any confident non-low classification or on a real `low` user turn. In-memory only; process restart is the right reset boundary. Raw level (pre-fallback) persisted to `transcript.thinking_level` for continuity. Model: `thinking_level` entry in `onnx_inference_service.MODEL_REGISTRY`, auto-downloaded from `chalie-ai/models` release tag `v0.9.0`. Resolved level mapped to `thinking_mode` string (`'low'` → `None`, `'medium'` → `'medium'`, `'high'` → `'high'`) and passed on every provider call via `Providers.send_messages()`. High mode additionally triggers a one-shot exploration pass in `_run_thinking_gate()` (same-job LLM call, `tools=[]`, result stored in `tool_calls` as `name='thinking'`, `ephemeral=0`); the exploration LLM call now sits inside the ACT-loop `MAX_TIMEOUT` envelope so a hung exploration cannot consume the wall-clock budget invisibly.
- **`contradiction_classifier_service.py`** — Pairwise memory-contradiction classifier (5-way: `temporal_change`, `true_contradiction`, `context_dependent`, `figurative`, `compatible`). `_build_onnx_input()` emits a compact JSON blob (`text_a`, `text_b`, `type_a`, `type_b`, `age_a_days`, `age_b_days`, `established_a`, `established_b`). Returns the MLP label directly. Log line: `[CONTRADICTION] MLP classification: …`. **Caller-supplied meta forwarding:** `check_new_trait(new_text, existing_text, source, existing_meta=None)` accepts the existing trait's metadata (`created_at`, `reinforcement_count`, `confidence`) and forwards it as `meta_b` so the model sees correct `age_b_days` and `established_b` instead of treating every well-reinforced trait as fresh & unestablished. The data_graph caller constructs `existing_meta` from the existing row dict it already holds. `_build_onnx_input` prefers an explicit `type` key over `source`, letting callers disambiguate (e.g. `source='chat'` with `type='trait'`). `sample_memories_for_reconcile` sets `type='trait'` explicitly so the batch reconcile path no longer falls through to the `'incoming'` default.
...
[User Input via WebSocket]
  → [WebSocket handler] spawns daemon thread
    → UserMessageProcessor(raw_input, metadata, on_narration).send(request_id)
      ...
      ├─ loop_start = time.time()  [MAX_TIMEOUT clock anchored BEFORE the gate
      │    so a hung exploration counts against the 900s deadline]
      ├─ _run_thinking_gate()  [user channel only]
      │    ThinkingLevelClassifierService.classify(user_turn, prev_level, channel)
      │    → level ∈ {low, medium, high}; confidence threshold 0.70
      │    → confidence < 0.70: sticky fallback (reuse prev_level or 'low' on cold start)
      │    → anti-demotion guard: head=low + prev∈{medium,high} → inherit prev_level
      │       chain-break: per-channel inherit counter (in-memory). After
      │       2 consecutive inherits the head wins and the channel demotes —
      │       prevents permanent 'high' lock-in on chit-chat follow-ups.
      │    → level persisted raw to transcript.thinking_level column
      │    → thinking_mode = None (low) | 'medium' | 'high'
      │    → level='high': one-shot exploration pass (same-job LLM, tools=[])
      │         result stored as tool_calls row (name='thinking', ephemeral=0)
      │         _wrap_with_exploration() prepends it to user_body each ACT iter
      │         (logs the [internal_exploration] marker exactly once per turn)
      │         excluded from all user-facing render by _NEVER_RENDER_IN_PREVIOUS
      ├─ ACT loop (up to 30 iter / 900s, deadline shared with the gate above):
      ...
      │        unsupported model → silent retry without flag
