...
- **Delegate tools** — `web_search` and `web_browse` each run a focused ACT turn on a `delegate:<name>` channel (`MessageProcessor.process(goal, config)`, `max_iterations=50`; no wall-clock deadline — the loop stops only on cooperative cancel or the iteration cap). On the user channel they run async (daemon thread, immediate ack, synthesis delivered back via `dispatch_message(..., hidden_input=True)` — the single chat chokepoint in `api/chat.py`); on any non-user channel they run synchronously inline. They replace the former `subagent` ability and its `SubagentProcessor`. See the "Delegate tools" section below.
...
- **Async (called on the user channel, `async=True` in params):** the daemon delivers the synthesis via `dispatch_message(result, channel="user", hidden_input=True)` — the single chat chokepoint — which starts a fresh user-channel turn; the raw result never lands in the transcript, only the synthesized assistant response. The user lists active delegates via `GET /chat/subagents/active` and cancels one via `POST /chat/subagent/<sub_id>/stop` (a legacy route name).
...
- `web_search`, `web_browse` — **delegate tools**: each runs a focused ACT turn on a `delegate:<name>` channel and (on the user channel) delivers its synthesis back asynchronously via `dispatch_message(..., hidden_input=True)`. See the "Delegate tools" section. They replace the removed `subagent` ability.
