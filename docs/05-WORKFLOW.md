...
1. **Consumer (`consumer.py`)** – Supervises worker processes and tracks shared state. Receives raw prompts via the `listener.py` entry point and enqueues them into the `prompt-queue`.
...
3. **Digest Worker (`workers/digest_worker.py`)** – Primary worker that:
   ...
   - Persists the prompt to a conversation file via `TopicConversationService`.
...
4. **Memory Chunker Worker (`workers/memory_chunker_worker.py`)** – Enriches individual exchanges with *memory chunks*:
   ...
   - Stores the enriched exchange back into the conversation JSON.
...
6. **Telegram Worker (`workers/telegram_worker.py`)** – Listens for Telegram updates, converts them to prompts, and enqueues them into the `prompt-queue`. Also sends responses back to Telegram.
...
10. **Redis / PostgreSQL** – Redis holds queue topics and small runtime state. PostgreSQL stores long-term episodic memories, semantic concepts, and routing decisions.
...
- **Mode-Specific Prompts** – Each mode (RESPOND, CLARIFY, ACKNOWLEDGE, ACT) has its own focused prompt template (~30-80 lines each), replacing the old combined 240-line prompt.
...
- **Confidence Calculation** – `TopicConversationService._calculate_new_confidence` uses a bounded reinforcement formula `new = current + (new_confidence - current) * 0.5`.
- **Renaming Strategy** – When a classifier proposes a `topic_update` with high confidence, the conversation file is renamed.
